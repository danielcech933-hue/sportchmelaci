-- Phase 1 security: authoritative slot engine.
-- IMPORTANT: the browser never supplies a grid or payout anymore.
...
BEGIN
  ...
  SELECT slot_czk INTO slot_bal FROM public.profiles WHERE id=uid FOR UPDATE;
  ...
  IF FOUND THEN
    ...
    IF sess.spins_remaining > 0 THEN
      ...
      base_bet := COALESCE(sess.base_bet,10);
    END IF;
  END IF;
  ...
  -- Server-side 5 x 3 result. No client-provided grid is accepted.
  ...
  FOR line_idx IN 1..5 LOOP
    ...
    IF cnt >= 3 THEN
      ...
      line_amount := round(payout * base_bet * bonus_mult,2);
      IF line_amount > 0 THEN
        total := total + line_amount;
        line_wins := line_wins || jsonb_build_array(jsonb_build_object(
          'line',line_idx-1,'symbol',base,'count',cnt,'amount',line_amount,'cells',cells
        ));
      END IF;
    END IF;
  END LOOP;
  ...
  IF scatter_count >= 3 THEN
    ...
    scatter_amount := round(payout * base_bet * bonus_mult,2);
  ...
  IF is_free THEN
    UPDATE public.slot_bonus_sessions
       SET spins_remaining=spins_remaining-1,
           total=round(public.slot_bonus_sessions.total+total,2),
           updated_at=now()
     WHERE user_id=uid;
  ...
  ELSIF scatter_count >= 3 THEN
    ...
    ON CONFLICT(user_id) DO UPDATE SET
      options=EXCLUDED.options,multiplier=NULL,base_bet=EXCLUDED.base_bet,
      spins_remaining=0,total=0,pending_pick=true,updated_at=now();
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'total',total,
    'multiplier_of_bet',CASE WHEN base_bet>0 THEN round(total/base_bet,2) ELSE 0 END,
