-- Fix: settle_match must pass its match id into the authoritative betting
-- ledger helper. The legacy 3-argument overload otherwise credits the wallet
-- without creating wallet_betting_ledger rows.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM jsonb_array_elements(m.bets) x
    WHERE x->>'status' IS NOT NULL AND x->>'status' <> 'open'
  ) THEN RETURN; END IF;
  ...
  IF unique_bettors < 2 THEN
    FOR b IN SELECT value FROM jsonb_array_elements(m.bets) LOOP
      IF COALESCE(b->>'status','open') = 'open' THEN
        PERFORM public.wallet_betting_credit(
          (b->>'userId')::uuid,
          COALESCE((b->>'amount')::numeric,0),
          'bet_refund',
          _match_id
        );
      END IF;
      new_b := b || jsonb_build_object(
        'status','refunded',
        'payout',COALESCE((b->>'amount')::numeric,0)
      );
  ...
  FOR b IN SELECT value FROM jsonb_array_elements(m.bets) LOOP
    IF b->>'pick' = winner THEN
      payout := ROUND(
        COALESCE((b->>'amount')::numeric,0) * total_pool / winning_stake,
        2
      );
      status := 'won';
      PERFORM public.wallet_betting_credit(
