-- FUT squad save hardening: starter slots and captain must be structurally valid.
-- Prevents clients from persisting arbitrary slot keys or a bench/reserve captain.
...
BEGIN
    ...
    IF length(trim(coalesce(_formation, ''))) < 3 OR length(trim(_formation)) > 32 THEN
    ...
    LOOP
        ...
        END IF;
        IF COALESCE(player->>'squad_role', 'STARTER') NOT IN ('STARTER', 'BENCH', 'RESERVE') THEN
            RAISE EXCEPTION 'invalid_squad_role';
        END IF;

        IF COALESCE(player->>'squad_role', 'STARTER') = 'STARTER'
           AND NOT (upper(trim(player->>'slot_key')) = ANY(valid_starter_slots)) THEN
