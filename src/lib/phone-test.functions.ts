...
export const getPhoneVerificationStatus = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return {
      ...
      pending: pending && pending.expiresAt > Date.now() ? { phone: pending.phone, expiresAt: pending.expiresAt, attempts: pending.attempts } : null,
    };
  });
...
export const startTestPhoneVerification = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (existing && existing.expiresAt > Date.now() && Date.now() - existing.issuedAt < START_COOLDOWN_MS) return { ok: true, reused: true, expiresAt: existing.expiresAt };
    const issuedAt = Date.now();
    const challenge = { phone, issuedAt, expiresAt: issuedAt + OTP_TTL_MS, attempts: 0 };
...
export const verifyTestPhone = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { phone: string; otp: string }) => ({ phone: String(input.phone).slice(0, 32), otp: String(input.otp).replace(/\D/g, "").slice(0, 6) }))
  .handler(async ({ data, context }) => {
    const { getServerUser, deriveTestOtp, readPending, safeOtpEqual, updateMetadata } = await import("./phone-test.server");
    ...
    await updateMetadata(context.userId, { ...metadata, sportchmelaci_verified_phone: { phone: pending.phone, verifiedAt, source: "test_admin" }, sportchmelaci_phone_challenge: null });
...
export const getAdminTestPhoneChallenges = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    const { isServerAdmin, getPendingAdminChallenges } = await import("./phone-test.server");
...
export const getPublicVerifiedPhone = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const { getServerUser, readVerified } = await import("./phone-test.server");
    const user = await getServerUser(data.userId);
    const verified = readVerified((user.user_metadata ?? {}) as Record<string, unknown>);
    if (!verified) return null;
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: pref } = await supabaseAdmin.from("phone_verifications").select("phone_public").eq("user_id", data.userId).maybeSingle();
...
export const setPublicPhoneForSelf = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await supabaseAdmin.from("phone_verifications").upsert({ user_id: context.userId, phone_public: data.enabled, updated_at: new Date().toISOString() });
