  ...
  return (
    <>
      ...
      <nav
        ...
        className={`pointer-events-none fixed inset-x-0 bottom-3 z-[9999] flex justify-center px-2 transition-[transform,opacity] duration-300 ease-out will-change-transform sm:bottom-5 ${
        ...
        >
          ...
          {visible.map((entry) => {
              ...
              return (
                <div key={entry.label} className="relative shrink-0">
                  <button
                    ...
                    aria-haspopup="menu"
                    title={entry.label}
                    className={`nav-chip group relative flex min-w-[3.4rem] shrink-0 touch-manipulation flex-col items-center gap-0.5 rounded-xl px-2 py-1.5 transition-[transform,color,background,box-shadow] duration-200 sm:min-w-[3.8rem] sm:px-2.5 ${
                      active || open
                  ...
                  >
                    <span className="relative inline-flex h-4 items-center justify-center">
                      <Icon className={`h-4 w-4 transition-transform duration-200 ${active || open ? "scale-110" : "group-hover:scale-110"}`} />
                      {(active || open) && <span aria-hidden className="absolute -bottom-1 h-0.5 w-3 rounded-full bg-primary shadow-[0_0_10px_var(--color-primary)]" />}
                    </span>
                    <span className="flex items-center gap-0.5 whitespace-nowrap text-[7px] font-semibold uppercase tracking-[0.08em] sm:text-[8px]">
                      {entry.label}
                  ...
                  {open && (
                    <div
                      role="menu"
                      onPointerDown={(e) => e.stopPropagation()}
                      className="absolute bottom-[calc(100%+0.55rem)] left-1/2 z-[10000] min-w-[11rem] -translate-x-1/2 rounded-2xl border border-primary/25 bg-background/95 p-1.5 shadow-2xl backdrop-blur-xl"
                    >
                        ...
                        {entry.items.map((item) => {
                          ...
                          return (
                            <Link
                              ...
                              onPointerDown={(e) => e.stopPropagation()}
                              role="menuitem"
                              className={`flex min-h-10 touch-manipulation items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold transition ${
            ...
            return (
              ...
              >
                <span className="relative inline-flex h-4 items-center justify-center">
                  <Icon className={`h-4 w-4 transition-transform duration-200 ${active ? "scale-110" : "group-hover:scale-110"} ${entry.fx === "trophy" ? "group-hover:trophy-pop" : ""}`} />
                  {active && <span aria-hidden className="absolute -bottom-1 h-0.5 w-3 rounded-full bg-primary shadow-[0_0_10px_var(--color-primary)]" />}
                  {entry.fx === "trophy" && (
