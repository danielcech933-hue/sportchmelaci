...
export function SocialHub({ compact = false, profileUserId }: Props) {
  ...
  return <section className={compact ? "space-y-3" : "space-y-5"}>
    ...
    {!profileUserId && <div className="aaa-card p-4 sm:p-5">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2"><span className="grid h-6 w-6 place-items-center rounded-lg border border-cyan-300/20 bg-cyan-300/[.06] text-cyan-200"><Sparkles className="h-3.5 w-3.5" /></span><div><div className="aaa-meta text-amber-200/70">SOCIAL FEED · LIVE COMMUNITY</div><h2 className="mt-1 font-display text-2xl tracking-[.12em] text-white">CO SE DĚJE NA CHMELOVCÍCH</h2></div></div></div>{user && <button type="button" onClick={() => setComposerOpen((value) => !value)} className="aaa-cta inline-flex min-h-10 items-center gap-2 px-4 py-2.5 text-[9px] font-black uppercase tracking-[.15em]"><Plus className="h-4 w-4" /> {composerOpen ? "Zavřít" : "Přidat příspěvek"}</button>}</div>
