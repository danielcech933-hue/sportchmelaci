...
export function DailyBonusWheel() {
  ...
  const canSpin = useMemo(() => hydrated && !spinning && (lastSpin === null || now - lastSpin >= COOLDOWN_MS), [hydrated, spinning, lastSpin, now]);
  const remainingMs = useMemo(() => !lastSpin ? 0 : Math.max(0, lastSpin + COOLDOWN_MS - now), [lastSpin, now]);
  ...
  const spin = useCallback(() => {
    ...
    timers.current.push(window.setTimeout(async () => {
      const reward = await addDollars(prize);
      if (!reward.ok) {
        setSpinning(false);
    ...
    <section className="relative overflow-hidden rounded-3xl border-2 border-hop-gold/40 bg-gradient-to-br from-black/90 via-zinc-950/80 to-black/95 p-6 backdrop-blur-2xl shadow-[0_0_50px_rgba(255,204,68,0.15)]">
      ...
      <div className="relative grid gap-6 lg:grid-cols-[auto_minmax(0,1fr)] lg:items-center">
        ...
        <div className="min-w-0 text-center lg:text-left space-y-3">
          ...
          <div className="pt-2"><button onClick={spin} disabled={!canSpin} className={`relative group overflow-hidden w-full sm:w-auto inline-flex items-center justify-center gap-3 rounded-2xl px-8 py-4 text-xs font-black uppercase tracking-[0.2em] transition-all duration-300 ${canSpin ? "border-2 border-amber-400 bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-600 text-black shadow-[0_0_30px_rgba(255,204,68,0.5)] hover:scale-105 active:scale-95 cursor-pointer" : "border border-zinc-800 bg-zinc-900/80 text-zinc-500 cursor-not-allowed opacity-70"}`}>{spinning ? <><Loader2 className="h-5 w-5 animate-spin text-black" /><span>NAČÍTÁM VÝHRU…</span></> : canSpin ? <><Flame className="h-5 w-5 text-black animate-pulse" /><span>ROZTOČIT KOLO ($5 - $50)</span></> : <><Gift className="h-5 w-5 text-zinc-500" /><span>DALŠÍ POKUS ZA {countdown}</span></>}</button></div>
        </div>
      </div>
    </section>
  );
