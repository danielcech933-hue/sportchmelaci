...
const BETS = [5, 10, 20, 50, 100, 250, 500];
    ...
    <div className={`relative overflow-hidden rounded-[32px] border bg-[#04070c] p-2 sm:p-4 ${frame}`}>
      ...
      <div className="relative z-10 rounded-[26px] border border-white/10 bg-black/50 p-3 backdrop-blur-xl sm:p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className={`font-mono text-[8px] font-black uppercase tracking-[.36em] ${thunder ? "text-amber-200" : "text-cyan-200"}`}>
              {meta.kicker} · SPORTCHMELÁCI ORIGINAL
            </div>
            <div className="mt-1 flex flex-wrap items-center gap-2">
              <h3 className="font-display text-3xl tracking-[.14em] text-white sm:text-4xl">{meta.title}</h3>
              <EpicBadge>CINEMATIC EDITION</EpicBadge>
            </div>
            <p className="mt-1 font-mono text-[9px] uppercase tracking-[.16em] text-white/40">
              {playerName} · SERVER RNG · SLOT CZK
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1 rounded-xl border border-white/10 bg-black/55 px-2 py-2 backdrop-blur">
              <button
                ...
                disabled={inBonus}
                className="grid h-7 w-7 place-items-center rounded-lg border border-white/10 text-white/70 disabled:opacity-30"
              >
                <Minus className="h-3.5 w-3.5" />
              </button>
              <div className="min-w-[64px] text-center">
                <div className="font-mono text-[7px] font-black uppercase tracking-[.18em] text-white/35">SÁZKA</div>
                <div className="font-display text-sm tracking-wider text-white">{bet} Kč</div>
              </div>
              <button
                type="button"
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.96 }}
              className={`inline-flex h-12 items-center gap-2 rounded-xl px-6 text-xs font-black uppercase tracking-[.18em] text-black transition disabled:cursor-not-allowed disabled:opacity-50 ${
                thunder
                  ? "bg-gradient-to-b from-amber-200 to-amber-500 shadow-[0_0_45px_-12px_rgba(255,204,68,.95)]"
                  : "bg-gradient-to-b from-cyan-200 to-cyan-500 shadow-[0_0_45px_-12px_rgba(90,220,255,.95)]"
              }`}
            >
              {spinning ? <RotateCw className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4" />}
              {spinning ? "TOČÍME" : inBonus ? "FREE SPIN" : "SPIN"}
            </motion.button>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 mt-3 grid gap-3 xl:grid-cols-[minmax(0,1fr)_18rem]">
        <motion.div
          animate={shaking ? shakeKeyframes(tier) : { x: 0, y: 0 }}
          transition={{ duration: 0.5 }}
          className="relative"
        >
          <motion.div
            animate={spinning ? { scale: 1.012 } : { scale: 1 }}
            transition={{ duration: 0.5 }}
            className={`relative overflow-hidden rounded-[28px] border-2 bg-black/65 p-2 shadow-[inset_0_1px_0_rgba(255,255,255,.12),0_35px_100px_-45px_rgba(0,0,0,.95)] sm:p-3 ${
              thunder ? "border-amber-200/45" : "border-cyan-200/40"
            }`}
          >
            ...
            <AnimatePresence>
              {spinning && (
                <motion.div
                  className="pointer-events-none absolute inset-x-0 top-0 z-30 h-2/3 bg-gradient-to-b from-transparent via-white/15 to-transparent blur-2xl"
            ...
            <div className="relative grid gap-1.5" style={{ gridTemplateColumns: `repeat(${meta.cols}, minmax(0,1fr))` }}>
              {grid.flatMap((column, c) =>
                column.map((symbol, r) => {
                  const key = `${c}-${r}`;
                  const isDivine = divine.has(key);
                  const isStrike = lightning.has(key);
                  const isMoney = !thunder && symbol === "fish_money";
                  return (
                    <motion.div
                      key={`${revealId}-${key}`}
                      initial={{ y: -26, opacity: 0.6, filter: "blur(6px)" }}
                      animate={{ y: 0, opacity: 1, filter: "blur(0px)" }}
                      transition={{ duration: 0.34, delay: c * 0.09 + r * 0.02, ease: [0.22, 1, 0.36, 1] }}
                      className={`relative flex aspect-[.88] items-center justify-center overflow-hidden rounded-xl border bg-gradient-to-b from-white/[.1] via-[#08121b] to-[#010204] ${
                        isStrike
                          ? "border-yellow-100 shadow-[0_0_42px_rgba(250,204,21,.95)]"
                          : isDivine
                            ? "border-amber-100 shadow-[0_0_38px_rgba(251,191,36,.8)]"
                            : isMoney
                              ? "border-amber-200/40 shadow-[0_0_26px_-6px_rgba(255,204,68,.6)]"
                              : "border-white/8"
                      }`}
                    >
                      ...
                      <EpicSymbolArt game={game} symbol={symbol} className="relative z-10" />
                      {isMoney && (
                        <motion.div
                          animate={{ scale: [1, 1.05, 1], opacity: [0.7, 1, 0.7] }}
                          transition={{ duration: 1.1, repeat: Infinity }}
                          className="absolute bottom-1.5 z-20 rounded-full border border-amber-200/40 bg-amber-300/10 px-2 py-0.5 font-mono text-[7px] font-black uppercase tracking-[.16em] text-amber-100"
                        >
                          MONEY
                        </motion.div>
                      )}
                      {isDivine && (
                        <motion.div
                          ...
                          className="pointer-events-none absolute inset-0 rounded-xl bg-[radial-gradient(circle,rgba(255,220,120,.3),transparent_60%)]"
                        />
                      )}
                      {isStrike && (
                        <motion.div
                          ...
                          className="pointer-events-none absolute inset-0 bg-[linear-gradient(135deg,transparent_40%,rgba(255,248,170,.95)_49%,transparent_58%)]"
                        />
                      )}
                    </motion.div>
                  );
            ...
            {/* inline win reveal above the reels */}
            <AnimatePresence>
              {tier && result?.total ? (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                >
                  <motion.div initial={{ scale: 0.6, y: 24 }} animate={{ scale: [1, 1.06, 1], y: 0 }} transition={{ duration: 0.7 }} className="text-center">
                    <div className="font-mono text-[9px] font-black uppercase tracking-[.36em] text-white/60">{result.feature}</div>
                    <div className={`font-display text-[clamp(26px,5vw,60px)] tracking-[.12em] ${thunder ? "text-amber-200" : "text-cyan-200"} drop-shadow-[0_0_30px_rgba(255,255,255,.5)]`}>
                      +{result.total.toLocaleString("cs-CZ")} CZK
                    </div>
                  </motion.div>
                </motion.div>
              ) : null}
        ...
        <aside className="space-y-2">
          <FeatureStat
            icon={thunder ? <Zap className="h-4 w-4 text-amber-300" /> : <Fish className="h-4 w-4 text-cyan-300" />}
            title={thunder ? "DIVINE REVEAL" : "MEGA CATCH"}
            value={thunder ? `${result?.divine_cells?.length ?? 0} zásahů` : `${(result?.bonus_collected ?? 0).toLocaleString("cs-CZ")} CZK`}
          />
          <FeatureStat
            icon={thunder ? <Gem className="h-4 w-4 text-amber-300" /> : <Waves className="h-4 w-4 text-cyan-300" />}
            title={thunder ? "LIGHTNING STRIKES" : "COLLECTOR"}
            value={thunder ? `${result?.lightning_cells?.length ?? 0}×` : `${collector} / 4`}
          />
          <FeatureStat
            icon={<Coins className="h-4 w-4 text-amber-300" />}
            title="POSLEDNÍ VÝHRA"
            value={`${Number(result?.multiplier_of_bet ?? 0).toFixed(1)}× sázky`}
          />

          {!thunder && (
            <div className="rounded-2xl border border-cyan-300/25 bg-cyan-300/[.04] p-3">
              <div className="flex items-center justify-between font-mono text-[8px] font-black uppercase tracking-[.18em] text-cyan-200">
                <span>COLLECTOR PROGRESS</span>
                <span>{collector}/4</span>
              </div>
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-black/50">
                <motion.div animate={{ width: `${Math.min(100, (collector / 4) * 100)}%` }} className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-emerald-300" />
              </div>
            </div>
          )}

          {inBonus && (
            <div className="rounded-2xl border border-amber-300/40 bg-amber-300/[.07] p-3 shadow-[0_0_35px_-18px_rgba(255,204,68,.9)]">
              <div className="flex items-center justify-between font-mono text-[8px] font-black uppercase tracking-[.17em] text-amber-200">
                <span>{featureFor(game, result?.bonus_mode).title}</span>
                <span>{freeSpins} SPINŮ</span>
              </div>
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-black/40">
                <motion.div animate={{ width: `${Math.min(100, Math.max(6, (freeSpins / 20) * 100))}%` }} className="h-full rounded-full bg-amber-300" />
              </div>
              <p className="mt-2 text-[10px] text-white/50">{featureFor(game, result?.bonus_mode).blurb}</p>
            </div>
          )}

          <div className="rounded-2xl border border-white/10 bg-black/45 p-3">
            <div className="font-mono text-[8px] font-black uppercase tracking-[.2em] text-white/45">FEATURE FLOW</div>
            <div className="mt-2 space-y-1.5">
              {EPIC_FEATURES[game].map((feature, index) => (
                <div key={feature.key} className="flex items-center gap-2 rounded-lg border border-white/8 bg-white/[.025] px-2.5 py-2">
                  <span className={`grid h-5 w-5 place-items-center rounded-full font-mono text-[8px] font-black ${thunder ? "bg-amber-300/10 text-amber-200" : "bg-cyan-300/10 text-cyan-200"}`}>
                    {index + 1}
                  </span>
                  <span className="font-mono text-[8px] font-black uppercase tracking-[.12em] text-white/65">{feature.title}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="relative z-10 mt-3">
        <EpicPaytablePanel game={game} />
      </div>

      <EpicBonusCinematic
        open={bonusOpen}
        game={game}
        mode={result?.bonus_mode}
        spins={freeSpins}
...
function FeatureStat({ icon, title, value }: { icon: React.ReactNode; title: string; value: string }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/45 p-3">
      <div className="grid h-9 w-9 place-items-center rounded-xl border border-white/10 bg-white/[.03]">{icon}</div>
      <div className="min-w-0">
        <div className="font-mono text-[7px] font-black uppercase tracking-[.18em] text-white/35">{title}</div>
        <div className="truncate font-display text-sm tracking-wider text-white">{value}</div>
      </div>
    </div>
  );
}
