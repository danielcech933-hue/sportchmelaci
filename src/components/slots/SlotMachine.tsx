...
function normalizeGrid(value: unknown): Grid {
  const fallback = spinGrid(); const source = Array.isArray(value) ? value : [];
  return Array.from({ length: REELS }, (_, reel) => { const column = Array.isArray(source[reel]) ? source[reel] : []; return Array.from({ length: ROWS }, (_, row) => column[row] ?? fallback[reel][row]); }) as Grid;
}
...
export function SlotMachine({ playerName, onExchange, onWin }: { playerName: string; onExchange?: () => void; onWin?: (multiplier: number) => void }) {
  ...
  const doSpin = useCallback(async () => {
    ...
    const serverMultiplier = Number(result.bonus_multiplier ?? 0); const serverGranted = Number(result.bonus_spins_granted ?? 0);
    if (isFree && serverMultiplier > 0) setBonusMultiplier(serverMultiplier);
    if (isFree && Number.isInteger(serverGranted) && serverGranted > 0) setBonusSpinsGranted(serverGranted);
    ...
    timers.current.push(window.setTimeout(() => {
      ...
      if (isFree && result.bonus_done) { setRecap({ total: Number(result.bonus_total ?? 0), spins: serverGranted > 0 ? serverGranted : bonusSpinsGranted, mult: serverMultiplier > 0 ? serverMultiplier : bonusMultiplier }); setBonusSpinsGranted(0); }
  ...
  useEffect(() => { if (busy || pickOptions || recap) return; if (freeSpinsLeft > 0) { const t = window.setTimeout(() => void doSpin(), 900); return () => window.clearTimeout(t); } if (autoLeft > 0) { const t = window.setTimeout(() => { setAutoLeft((n) => n - 1); void doSpin(); }, 700); return () => window.clearTimeout(t); } }, [busy, freeSpinsLeft, autoLeft, pickOptions, recap, doSpin]);
  ...
  return (
    <div className="relative" data-slot-spinning={busy ? "true" : "false"} data-slot-anticipation={anticipation ? "true" : "false"}>
      ...
      <div className="mb-3 flex items-center justify-between gap-3 rounded-2xl border border-hop-gold/30 bg-black/40 px-4 py-2.5 backdrop-blur-xl"><div><p className="font-display text-base tracking-[.18em] slot-gold-text">CHMELOVCI CUP</p><p className="font-mono text-[8px] uppercase tracking-[.24em] text-hop-neon/60">SPORTOVNÍ SLOT · 5 VÁLCŮ / 3 ŘADY</p></div><div className="flex items-center gap-2">{anticipation && <motion.span className="rounded-full border border-hop-gold/50 bg-hop-gold/10 px-2.5 py-1 font-mono text-[8px] font-black uppercase tracking-[.18em] text-hop-gold" initial={{ opacity: 0, scale: .9 }} animate={{ opacity: [0.5, 1, 0.5], scale: [0.98, 1.03, 0.98] }} transition={{ duration: .8, repeat: Infinity }}>SCATTER ANTICIPACE</motion.span>}<div className="hidden rounded-lg border border-hop-gold/20 bg-hop-gold/5 px-3 py-1 text-right sm:block"><p className="font-mono text-[8px] uppercase tracking-widest text-hop-gold/60">STATUS</p><p className={`font-display text-xs ${busy ? "text-hop-gold" : "text-hop-neon"}`}>{busy ? "TOČÍME" : "READY"}</p></div></div></div>
