...
type SpinResult = {
  ...
  feature: string;
  slot_czk: number;
  bonus?: { type?: string; spins?: number; multiplier?: number };
};
...
export function VariantSlotMachine({ game, playerName }: { game: SlotVariantId; playerName: string }) {
  ...
  return (
    <div className="relative rounded-[1.6rem] border border-white/10 bg-black/45 p-3 sm:p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/10 bg-black/35 px-3 py-2">
        ...
        <div className="text-right"><p className="font-mono text-[8px] uppercase tracking-[.22em] text-white/40">Slot CZK</p><motion.p key={slotCZK} initial={{ scale: .9, color: "#ffffff" }} animate={{ scale: 1, color: "#ffcc44" }} className="font-mono text-sm font-black">{slotCZK.toLocaleString("cs-CZ")} Kč</motion.p></div>
      </div>

      <motion.div animate={spinning ? { scale: [1, 1.01, .995, 1], filter: ["brightness(1)", "brightness(1.4)", "brightness(.9)", "brightness(1)"] } : { scale: 1, filter: "brightness(1)" }} transition={{ duration: .8, repeat: spinning ? Infinity : 0 }} className={cn("mx-auto grid max-w-4xl gap-1.5 rounded-2xl border bg-black/55 p-2 transition", accent)} style={{ gridTemplateColumns: `repeat(${meta.columns}, minmax(0, 1fr))` }}>
        {grid.flatMap((col, c) => col.map((symbol, r) => {
          const active = !spinning && result?.grid?.[c]?.[r] === symbol;
          return <motion.div key={`${c}-${r}`} initial={{ scale: .94, opacity: .8 }} animate={{ scale: spinning ? [1, 0.96, 1.03, 1] : 1, opacity: 1 }} transition={{ duration: .42, delay: spinning ? (c * .025) + (r * .01) : 0, repeat: spinning ? Infinity : 0 }} className={cn("flex aspect-square items-center justify-center rounded-xl border border-white/5 bg-gradient-to-b from-white/[0.07] to-black/60 font-display text-lg text-white sm:text-2xl", spinning ? "animate-pulse" : "", active && result?.total ? "ring-2 ring-hop-gold/70 shadow-[0_0_25px_-8px_rgba(255,204,68,.95)]" : "")}>
            <span className={symbol === "wild" || symbol === "mystery" ? "text-hop-gold drop-shadow-[0_0_18px_rgba(255,204,68,.95)]" : ""}>{meta.symbols[symbol] ?? symbol}</span>
          </motion.div>;
        }))}
      </motion.div>
      ...
      <AnimatePresence mode="wait">{result && <motion.div key={`${result.game_id}-${result.total}-${result.feature}`} initial={{ opacity: 0, y: 16, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="mt-3 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-white/10 bg-black/30 p-3"><Trophy className="h-4 w-4 text-hop-gold" /><p className="mt-1 font-mono text-[8px] uppercase tracking-[.14em] text-white/35">Výhra</p><p className="font-display text-lg text-white">{result.total.toLocaleString("cs-CZ")} CZK</p></div><div className="rounded-xl border border-white/10 bg-black/30 p-3"><Sparkles className="h-4 w-4 text-hop-neon" /><p className="mt-1 font-mono text-[8px] uppercase tracking-[.14em] text-white/35">Feature</p><p className="font-display text-sm text-white">{result.feature}</p></div><div className="rounded-xl border border-white/10 bg-black/30 p-3"><CircleDollarSignIcon /><p className="mt-1 font-mono text-[8px] uppercase tracking-[.14em] text-white/35">Násobitel</p><p className="font-display text-lg text-white">{Number(result.multiplier_of_bet || 0).toFixed(2)}×</p></div></motion.div>}</AnimatePresence>
      <VariantBonusOverlay type={bonusReveal?.type} spins={bonusReveal?.spins} multiplier={bonusReveal?.multiplier} game={game} onDone={() => setBonusReveal(null)} />
    </div>
  );
}
