...
export function CaseOpeningLobby() {
  ...
  return (
    <main className="min-h-screen bg-[#02060b] pb-28 text-white">
      <div className="mx-auto max-w-7xl px-3 pt-6 sm:px-5 lg:pt-10">
        <section className="relative overflow-hidden rounded-[32px] border border-white/10 bg-[radial-gradient(circle_at_18%_0%,rgba(83,193,255,.14),transparent_28%),radial-gradient(circle_at_84%_8%,rgba(211,99,255,.12),transparent_26%),linear-gradient(180deg,#09131d,#02060b)] p-4 shadow-[0_50px_150px_-70px_rgba(60,180,255,.5)] sm:p-7">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="rounded-2xl border border-cyan-300/25 bg-cyan-300/5 p-4 lg:min-w-[280px]"><div className="font-mono text-[8px] font-black uppercase tracking-[.2em] text-cyan-200/55">PLAYER BANKROLL</div><div className="mt-1 font-display text-2xl tracking-[.07em]">{dollars(cash)}</div><div className="mt-1 flex items-center gap-2 text-xs text-white/40"><Sparkles className="h-3.5 w-3.5 text-cyan-300" /> {nickname} · VIRTUAL ECONOMY</div></div>
          </div>
        </section>

        <div className="mt-5 flex gap-2 overflow-x-auto pb-1">{FILTERS.map((item) => <button key={item} type="button" onClick={() => setFilter(item)} className={cn("shrink-0 rounded-xl border px-3 py-2 font-mono text-[8px] font-black uppercase tracking-[.16em] transition", filter === item ? "border-cyan-200/45 bg-cyan-200/10 text-cyan-100" : "border-white/10 bg-white/[.02] text-white/40 hover:text-white/70")}>{item}</button>)}</div>

        <section className="mt-4 grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {visibleCases.map((cfg) => {
            const Icon = cfg.icon; const busy = opening === cfg.id;
            return <motion.article key={cfg.id} whileHover={{ y: -5 }} className={cn("relative overflow-hidden rounded-[28px] border bg-[#071018] p-4 shadow-2xl", cfg.accent === "cyan" ? "border-cyan-300/25" : cfg.accent === "violet" ? "border-violet-300/25" : cfg.accent === "gold" ? "border-amber-300/30" : "border-white/10")}>
              ...
              <div className="relative mt-4 rounded-2xl border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,.05),rgba(0,0,0,.18))] p-5"><div className="font-mono text-[8px] font-black uppercase tracking-[.3em] text-white/30">CASE {CASES.findIndex((x) => x.id === cfg.id) + 1} / 10</div><div className="mt-2 font-display text-2xl tracking-[.08em]">{cfg.name}</div><p className="mt-2 min-h-10 text-xs leading-relaxed text-white/40">{cfg.description}</p><div className="mt-4 grid grid-cols-2 gap-2"><Stat label="ENTRY" value={dollars(cfg.cost)} /><Stat label="POOL" value="50+ companies" /></div></div>
        ...
        <section className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="rounded-[26px] border border-white/10 bg-[#071018] p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><div><div className="font-mono text-[8px] font-black uppercase tracking-[.28em] text-white/35">COLLECTIBLE INVENTORY</div><h2 className="mt-1 font-display text-2xl tracking-[.09em]">YOUR SHARE VAULT</h2></div><div className="rounded-xl border border-white/10 bg-black/25 px-3 py-2 font-mono text-[8px] font-black uppercase tracking-[.15em] text-white/45">{inventory.length} / 100</div></div>{inventory.length ? <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{inventory.slice(0,12).map((row) => <InventoryCard key={row.serial} row={row} />)}</div> : <div className="mt-4 rounded-2xl border border-dashed border-white/10 p-8 text-center text-sm text-white/30">Vault je zatím prázdný. Otevři první case.</div>}</div>
      ...
      <AnimatePresence>{drop && <motion.div className="fixed inset-0 z-[11100] grid place-items-center bg-black/80 px-4 backdrop-blur-lg" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setDrop(null)}><motion.div initial={{ scale: .75, y: 30 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-2xl rounded-[32px] border border-cyan-200/30 bg-[radial-gradient(circle_at_50%_0%,rgba(83,193,255,.14),transparent_38%),#081018] p-7 text-center shadow-[0_0_150px_rgba(83,193,255,.2)]" onClick={(e) => e.stopPropagation()}><div className="font-mono text-[9px] font-black uppercase tracking-[.34em] text-cyan-200/55">CASE REVEALED · VIRTUAL COLLECTIBLE</div><div className="mt-3 font-display text-4xl tracking-[.1em] sm:text-5xl">{drop.company_name}</div><div className="mt-2 font-mono text-sm font-black tracking-[.22em] text-white/45">{drop.ticker} · {drop.sector}</div><div className="mt-6 grid grid-cols-2 gap-3"><RevealStat label="SHARE LOT" value={shares(drop.share_count)} /><RevealStat label="RARITY SCORE" value={String(drop.rarity_score)} /></div><div className={cn("mx-auto mt-5 inline-flex rounded-full border px-4 py-2 font-mono text-[9px] font-black uppercase tracking-[.23em]", RARITY_STYLES[drop.rarity] ?? RARITY_STYLES.COMMON)}>{drop.rarity}</div><div className="mt-5 rounded-2xl border border-white/10 bg-black/25 p-4"><div className="font-mono text-[8px] uppercase tracking-[.18em] text-white/30">SERIAL</div><div className="mt-1 font-mono text-sm font-black tracking-[.18em] text-white/70">{drop.serial}</div></div><p className="mt-5 text-xs leading-relaxed text-white/35">Vzácnost = kombinace rarity společnosti a velikosti share lotu. Čím vzácnější společnost a menší počet akcií, tím vyšší tier. Jedná se pouze o virtuální herní collectible.</p><button type="button" onClick={() => setDrop(null)} className="mt-6 rounded-xl border border-cyan-200/25 bg-cyan-200/10 px-5 py-3 font-mono text-[9px] font-black uppercase tracking-[.18em] text-cyan-100">ADD TO VAULT</button></motion.div></motion.div>}</AnimatePresence>
    </main>
  );
}
