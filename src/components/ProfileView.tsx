...
function playsInMatch(nickname: string, m: Match): boolean { return sideOf(nickname, m) !== null; }
function matchOutcome(nickname: string | null, m: Match): "win" | "loss" | null {
  if (!nickname) return null; const w = winnerSideOf(m); if (!w) return null; const side = sideOf(nickname, m); if (!side) return null; return side === w ? "win" : "loss";
}

export function ProfileView({ userId }: { userId?: string }) {
  const { user, nickname: myNickname, avatarPath: myAvatar, refreshProfile, loading: authLoading } = useAuth();
  const { openChat } = useDm(); const { userDollars, slotCZK } = useWallet();
  const targetId = userId ?? user?.id ?? null; const isSelf = !!targetId && targetId === user?.id;
  ...
  const myMatches = useMemo(() => { if (!nickname) return []; return matches.filter((m) => playsInMatch(nickname, m)).sort((a, b) => (b.endedAt ?? b.startedAt) - (a.endedAt ?? a.startedAt)); }, [matches, nickname]);
  const myBets: BetRow[] = useMemo(() => { if (!nickname) return []; const rows: BetRow[] = []; for (const m of matches) { const w = winnerSideOf(m); for (const b of m.bets ?? []) if (b.bettor?.toLowerCase() === nickname.toLowerCase()) rows.push({ ...b, matchId: m.id, match: m, status: w ? (b.pick === w ? "won" : "lost") : "open" }); } return rows.sort((a, b) => (b.createdAt ?? 0) - (a.createdAt ?? 0)); }, [matches, nickname]);

  const [ledgerNet, setLedgerNet] = useState(0); const [ledgerLoaded, setLedgerLoaded] = useState(false);
  useEffect(() => { if (!isSelf) { setLedgerNet(0); setLedgerLoaded(false); return; } let active = true; setLedgerLoaded(false); supabase.rpc("get_my_betting_ledger", { _limit: 200 }).then(({ data, error }) => { if (!active) return; if (error) { setLedgerLoaded(false); return; } const settledRows = (data ?? []).filter((row: any) => row.kind === "bet_payout" || row.kind === "bet_refund"); const payoutNet = settledRows.reduce((sum: number, row: any) => sum + Number(row.amount ?? 0), 0); const settledMatchIds = new Set(settledRows.map((row: any) => row.match_id ?? row.metadata?.match_id).filter(Boolean).map(String)); const settledStake = myBets.reduce((sum, bet) => settledMatchIds.has(String(bet.matchId)) ? sum + (bet.status === "won" || bet.status === "lost" ? Number(bet.amount ?? 0) : 0) : sum, 0); setLedgerNet(Math.round((payoutNet - settledStake) * 100) / 100); setLedgerLoaded(true); }); return () => { active = false; }; }, [isSelf, targetId, myBets]);

  const stats = useMemo(() => { let betWon = 0, betLost = 0, betOpen = 0, biggestBet = 0; for (const b of myBets) { biggestBet = Math.max(biggestBet, b.amount ?? 0); if (b.status === "won") betWon++; else if (b.status === "lost") betLost++; else betOpen++; } const split = playerSplitStats(matches, nickname); const sports = new Set(myMatches.map((m) => m.sport)); return { solo: split.solo, team: split.team, total: split.overall.total, victories: split.overall.wins, losses: split.overall.losses, betWon, betLost, betOpen, moneyNet: ledgerLoaded ? ledgerNet : 0, biggestBet, sports: sports.size }; }, [matches, myMatches, myBets, nickname, ledgerLoaded, ledgerNet]);
  ...
  const heroFx = useMemo(() => { const fx = ["avatar-fx-pulse", "avatar-fx-spin", "avatar-fx-glitch", "avatar-fx-float", "avatar-fx-hue"]; return fx[Math.floor(Math.random() * fx.length)]; }, [targetId]);
  ...
  return <main className="relative mx-auto max-w-6xl px-3 py-6 sm:px-4 sm:py-10">
    <section className="relative overflow-hidden rounded-[2rem] border border-primary/30 bg-background/70 shadow-[0_30px_90px_-55px_var(--color-primary)] backdrop-blur-md"><img src={heroImg} alt="" width={1600} height={720} className="h-48 w-full object-cover opacity-55 sm:h-72" /><div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-background via-background/55 to-transparent" /><div className="pointer-events-none absolute inset-0 grid-bg opacity-20" /><div className="absolute inset-x-0 top-0 flex items-center justify-between p-4 sm:p-6"><div className="inline-flex items-center gap-2 rounded-full border border-primary/25 bg-background/45 px-3 py-1.5 text-[9px] font-semibold uppercase tracking-[0.25em] text-primary backdrop-blur">PLAYER // PROFILE</div><div className="inline-flex items-center gap-2 rounded-full border border-accent/20 bg-background/45 px-3 py-1.5 text-[9px] uppercase tracking-[0.2em] text-muted-foreground backdrop-blur"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-accent" /> {isSelf ? "Aktivní hráč" : "SportChmeláci"}</div></div><div className="absolute inset-x-0 bottom-0 flex items-end gap-4 p-4 sm:gap-6 sm:p-8"><div className={`shrink-0 rounded-full ring-2 ring-primary/40 ring-offset-4 ring-offset-background/20 ${heroFx}`}><Avatar path={avatarPath} nickname={nickname} size={104} /></div><div className="min-w-0 pb-1"><div className="text-[10px] uppercase tracking-[0.3em] text-primary/80">{isSelf ? "Můj profil" : "Profil hráče"}</div><h1 className="mt-1 truncate font-display text-4xl tracking-wider neon-text sm:text-7xl"><span className="mr-2 align-middle text-2xl sm:text-4xl">{playerEmoji(nickname)}</span><span className="text-primary">{nickname ?? "PLAYER"}</span></h1><p className="mt-1 text-[10px] uppercase tracking-[0.22em] text-muted-foreground sm:text-xs">Zápasy · výkon · betting activity</p></div></div></section>
    ...
    <ProfilePlayerCard nickname={nickname} stats={stats} />
    <ProfileAchievements userId={targetId} />
