...
function buildRoster(row: SquadRow): Roster {
  ...
  for (const player of row.players ?? []) {
    ...
    } else if (player.squad_role === "BENCH" && !seenBench.has(player.user_card_id)) {
      seenBench.add(player.user_card_id);
      bench.push(player.user_card_id);
    } else if (player.squad_role === "RESERVE" && !seenReserves.has(player.user_card_id)) {
      seenReserves.add(player.user_card_id);
...
export function SquadBuilder({ cards }: { cards: UtOwnedCard[] }) {
  ...
  const hydrate = (row: SquadRow, preserveLocal = false) => {
    const nextRoster = buildRoster(row);
    setSquadId(row.id);
    setVersion(row.version);
    if (!preserveLocal) setRoster(nextRoster);
    setSavedRoster(nextRoster);
    setMetrics((current) => ({ ...current, starting_xi: Object.values(nextRoster.starting).filter(Boolean).length }));
  ...
  const usedIds = useMemo(() => {
    const ids = [...Object.values(roster.starting), ...roster.bench, ...roster.reserves].filter(Boolean) as string[];
    return new Set(ids);
  }, [roster]);
  ...
  const candidates = active
    ? cards.filter((card) => !usedIds.has(card.id) || card.id === roster.starting[active.key])
    ...
    : (activeRole ? cards.filter((card) => !usedIds.has(card.id)).sort((a, b) => b.card.rating - a.card.rating) : []);
  ...
  function assign(cardId: string) {
    if (activeSlot) {
      setRoster((current) => {
        const next = { ...current, starting: { ...current.starting } };
        for (const key of Object.keys(next.starting) as SlotKey[]) if (next.starting[key] === cardId) next.starting[key] = null;
        next.starting[activeSlot] = cardId;
        return next;
      });
    } else if (activeRole === "BENCH" && activeIndex !== null) {
      setRoster((current) => {
        const nextBench = [...current.bench];
        if (activeIndex >= 7) return current;
        while (nextBench.length <= activeIndex) nextBench.push("");
        for (let i = 0; i < nextBench.length; i += 1) if (nextBench[i] === cardId) nextBench[i] = "";
        nextBench[activeIndex] = cardId;
        return { ...current, bench: nextBench.filter(Boolean) };
      });
    } else if (activeRole === "RESERVE" && activeIndex !== null) {
      setRoster((current) => {
        const nextReserves = [...current.reserves];
        if (activeIndex >= 5) return current;
        while (nextReserves.length <= activeIndex) nextReserves.push("");
        for (let i = 0; i < nextReserves.length; i += 1) if (nextReserves[i] === cardId) nextReserves[i] = "";
        nextReserves[activeIndex] = cardId;
        return { ...current, reserves: nextReserves.filter(Boolean) };
      });
    }
    setSaved(false);
    clearSelection();
