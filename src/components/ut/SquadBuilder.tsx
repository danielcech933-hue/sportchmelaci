...
type Roster = { starting: Squad; bench: string[]; reserves: string[]; captainId: string | null };
...
const EMPTY_ROSTER: Roster = { starting: EMPTY_SQUAD, bench: [], reserves: [], captainId: null };
...
function buildRoster(row: SquadRow): Roster {
  ...
  for (const player of row.players ?? []) {
    ...
    if (player.squad_role === "STARTER") {
      ...
      if (key in starting) {
        starting[key] = player.user_card_id;
...
export function SquadBuilder({ cards }: { cards: UtOwnedCard[] }) {
  ...
  function removeFromAll(id: string, current: Roster): Roster {
    ...
    return { starting, bench: current.bench.filter((v) => v !== id), reserves: current.reserves.filter((v) => v !== id), captainId: current.captainId === id ? null : current.captainId };
  }
  ...
  async function save() {
    ...
    if (!roster.captainId || !Object.values(roster.starting).includes(roster.captainId)) { setError("Vyber kapitána ze základní sestavy."); return; }
    setSaving(true); setError(null); setSaved(false);
    try {
      const players = [
        ...SLOTS.map((slot) => ({ user_card_id: roster.starting[slot.key], slot_key: slot.key, position: slot.label, squad_role: "STARTER", is_captain: roster.captainId === roster.starting[slot.key] })).filter((p) => p.user_card_id),
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
      <section className="rounded-3xl border border-primary/25 bg-background/60 p-3 sm:p-5">
        ...
        <div className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-emerald-300/25 bg-[radial-gradient(circle_at_center,rgba(16,185,129,0.12),transparent_60%),linear-gradient(180deg,rgba(5,46,22,0.8),rgba(2,15,9,0.95))]">
          ...
          {SLOTS.map((slot) => { const owned = cards.find((card) => card.id === roster.starting[slot.key]); const isCaptain = roster.captainId === roster.starting[slot.key]; return <button key={slot.key} type="button" onClick={() => { setActiveSlot(slot.key); setActiveRole(null); setActiveIndex(null); }} onContextMenu={(event) => { event.preventDefault(); clearStartingSlot(slot.key); }} className={cn("absolute z-10 -translate-x-1/2 -translate-y-1/2 rounded-xl border p-1 transition hover:scale-105", owned ? "border-primary/50 bg-black/30" : "border-dashed border-emerald-200/30 bg-black/20")} style={{ left: `${slot.x}%`, top: `${slot.y}%` }} title={owned ? `${owned.card.name} · klik pro výběr, pravým tlačítkem odebrat` : `Přidat ${slot.label}`}>{owned ? <UltimateCard card={owned.card} size="sm" badge={isCaptain ? "C" : slot.label} /> : <span className="flex h-[118px] w-[104px] flex-col items-center justify-center gap-1 text-emerald-100/60"><span className="text-2xl">+</span><span className="font-mono text-[9px] uppercase tracking-widest">{slot.label}</span></span>}</button>; })}
        </div>
