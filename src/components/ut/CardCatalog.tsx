    ...
    <div className="space-y-4">
      <div className="rounded-3xl border border-primary/20 bg-background/45 p-4 sm:p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-primary/70">Ultimate Team · Card Database</p>
            <h2 className="mt-1 font-display text-2xl uppercase tracking-[0.08em]">Katalog všech karet</h2>
            <p className="mt-1 text-xs text-muted-foreground">Kompletní přehled karet, které lze v Ultimate Teamu získat.</p>
          </div>
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-[minmax(0,1fr)_auto_auto_auto]">
          <label className="relative">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Hledat hráče, klub, národ, ligu…" className="w-full rounded-xl border border-border/60 bg-background/60 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-primary/60" />
          </label>
        ...
        <div className="mt-3 flex items-center gap-2 rounded-xl border border-border/50 bg-background/40 px-3 py-2">
          <SlidersHorizontal className="h-3.5 w-3.5 text-primary" />
          <span className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">Min. OVR</span>
          <input type="range" min={0} max={99} step={1} value={minRating} onChange={(e) => setMinRating(Number(e.target.value))} className="flex-1 accent-[var(--color-primary)]" />
          <span className="w-8 text-right font-mono text-xs text-primary">{minRating || "ALL"}</span>
        </div>
      </div>
        ...
        <div className="overflow-hidden rounded-3xl border border-primary/15 bg-background/40">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[980px] text-left">
              <thead className="border-b border-primary/15 bg-primary/5">
                <tr className="font-mono text-[9px] uppercase tracking-[0.18em] text-muted-foreground">
                  {['Karta','OVR','Pozice','Klub','Liga','Národ','Rarita','PAC','SHO','PAS','DRI','DEF','PHY','Stav'].map((h) => <th key={h} className="px-3 py-3">{h}</th>)}
                </tr>
              </thead>
              <tbody className="divide-y divide-primary/10">
                {filtered.map((card) => {
                  const isOwned = owned.has(card.id);
                  return <tr key={card.id} className={cn("transition hover:bg-primary/5", isOwned && "bg-accent/[0.035]")}>
                    <td className="px-3 py-3"><div className="font-semibold text-foreground">{card.name}</div><div className="text-[10px] text-muted-foreground">{card.card_type}{card.campaign ? ` · ${card.campaign}` : ""}</div></td>
                    <td className="px-3 py-3 font-display text-lg text-primary">{card.rating}</td>
                    <td className="px-3 py-3 font-mono text-xs">{card.position}</td>
                    <td className="px-3 py-3 text-xs">{card.club}</td>
                    <td className="px-3 py-3 text-xs text-muted-foreground">{card.league}</td>
                    <td className="px-3 py-3 text-xs">{card.nation}</td>
                    <td className="px-3 py-3 text-[10px] font-bold uppercase text-accent">{card.rarity}</td>
                    {[card.pac, card.sho, card.pas, card.dri, card.def, card.phy].map((v, i) => <td key={i} className="px-3 py-3 font-mono text-xs">{v}</td>)}
                    <td className="px-3 py-3"><span className={cn("inline-flex rounded-full border px-2 py-1 font-mono text-[9px] uppercase tracking-widest", isOwned ? "border-accent/40 bg-accent/10 text-accent" : "border-border/60 text-muted-foreground")}>{isOwned ? "VLASTNÍM" : "NEVLASTNÍM"}</span></td>
                  </tr>;
                })}
              </tbody>
            </table>
