import { NATION_FLAG, chibiFor } from "@/lib/cards";
import { rarityMeta } from "@/lib/ut";
import { cn } from "@/lib/utils";
import type { UtCard } from "@/types/ut";
...
/** Ultimate Team karta s rarity rámem, PlayStyles a chibi hráčem. */
export function UltimateCard({ card, size = "md", className, onClick, selected, badge }: Props) {
  ...
  return (
    ...
    >
      ...
      {meta.walkout && (
        <div className="pointer-events-none absolute inset-0 animate-pulse bg-[conic-gradient(from_180deg,transparent,rgba(255,255,255,0.18),transparent)]" />
      )}
      <div className="relative flex items-start justify-between">
        <div className="leading-none">
          ...
          <p className={cn("mt-1 max-w-[56px] truncate font-mono uppercase opacity-70", meta.text)}>{card.club}</p>
        </div>
        <img
          src={chibiFor({ ...card, cardType: "gold" } as never)}
      ...
      {size !== "sm" && card.playstyles.length > 0 && (
        <p className={cn("relative mt-1 truncate font-mono text-[8px] uppercase tracking-[0.14em] opacity-70", meta.text)}>
          ⚡ {card.playstyles.slice(0, 2).join(" · ")}
        </p>
      )}
      ...
      {badge && (
        <span className="absolute bottom-1 right-1 rounded bg-black/70 px-1 font-mono text-[8px] uppercase tracking-widest text-primary">
          {badge}
        </span>
      )}
    </button>
