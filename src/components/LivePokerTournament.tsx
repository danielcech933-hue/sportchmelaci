    ...
    <div className="space-y-6">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <MagneticText
              ...
              className="font-display text-3xl font-black tracking-wider text-amber-200 sm:text-4xl"
            />
            <p className="text-xs text-amber-200/60">
              Připoj se k probíhajícímu stolu nebo založ vlastní turnaj pro přátele.
            </p>
          </div>
          <button
        ...
        {creating && (
          ...
          >
            <div className="rounded-3xl border border-amber-500/30 bg-black/80 p-6 backdrop-blur-2xl shadow-xl">
              <h3 className="mb-4 font-mono text-xs font-black uppercase tracking-widest text-amber-400 flex items-center gap-2">
                <Crown className="h-4 w-4" /> Parametry nového stolu
              </h3>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <label className="space-y-1.5">
                  <span className="font-mono text-[10px] uppercase tracking-widest text-zinc-400">Název Turnaje</span>
                  <input
                    value={form.name}
                    placeholder="Např. Royal High Rollers"
                    onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                    className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-amber-100 placeholder:text-zinc-600 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500"
                  />
                </label>
                <label className="space-y-1.5">
                  <span className="font-mono text-[10px] uppercase tracking-widest text-zinc-400">Buy-In ($)</span>
                  <input
                    type="number"
                    min={0}
                    value={form.buyIn}
                    onChange={(e) => setForm((f) => ({ ...f, buyIn: Math.max(0, Number(e.target.value) || 0) }))}
                    className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-amber-100 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500"
                  />
                </label>
                <label className="space-y-1.5">
              ...
              <button
                onClick={create}
                className="mt-5 w-full rounded-xl bg-gradient-to-r from-amber-500 via-amber-400 to-amber-500 py-3 font-mono text-xs font-black uppercase tracking-widest text-black shadow-lg shadow-amber-500/20 hover:brightness-110 active:scale-[0.99]"
              >
                Potvrdit a Založit Turnaj
              </button>
            </div>
          </motion.div>
        )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {tournaments.length === 0 && (
          <div className="col-span-full flex flex-col items-center justify-center rounded-3xl border border-dashed border-white/10 p-12 text-center">
            <Coins className="h-10 w-10 text-zinc-600 mb-2" />
            <p className="text-sm font-medium text-zinc-400">Zatím nebyly založeny žádné aktivní turnaje.</p>
            <p className="text-xs text-zinc-600">Buď prvním, kdo vytvoří pokerový stůl!</p>
          </div>
        )}
        {tournaments.map((t) => {
          ...
          return (
            <TiltCard
              key={t.id}
              className="group relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b from-zinc-900/90 to-black/90 p-5 backdrop-blur-xl transition-all duration-300 hover:border-amber-500/40 hover:shadow-2xl hover:shadow-amber-500/10"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <FxText glitch className="font-display text-xl font-bold tracking-wide text-amber-100">
                    {t.name}
                  </FxText>
              ...
              <div className="mt-6 flex items-center justify-between border-t border-white/5 pt-4 text-xs">
                <span className="inline-flex items-center gap-1.5 font-mono text-zinc-400">
                  ...
                  {mySeats.length} / {t.max_players} hráčů
                </span>
                <span
                  className={cn(
                    "inline-flex items-center gap-1 font-mono text-[11px] font-bold uppercase tracking-wider",
                    t.hand && t.hand.stage !== "done" ? "text-emerald-400" : "text-amber-400/80",
                  )}
                >
                  <span
                    className={cn(
                      "h-1.5 w-1.5 rounded-full",
                      t.hand && t.hand.stage !== "done" ? "bg-emerald-400 animate-ping" : "bg-amber-400",
                    )}
                  />
                  {t.hand && t.hand.stage !== "done" ? "Hra probíhá" : "Čeká v lobby"}
                </span>
              </div>
              ...
              <button
                ...
                className={cn(
                  "mt-4 w-full rounded-xl py-2.5 font-mono text-xs font-black uppercase tracking-widest transition-all",
                  seated
              ...
              >
                {seated ? "Vrátit se ke stolu" : "Připojit se ke stolu"}
              </button>
            </TiltCard>
          );
        })}
...
function HoleCard({ card, hidden, delay = 0 }: { card?: Card; hidden?: boolean; delay?: number }) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative flex h-16 w-11 sm:h-20 sm:w-14 select-none items-center justify-center rounded-xl border text-base font-black shadow-xl backdrop-blur-md transition-all duration-300",
        hidden || !card
          ? "border-amber-500/40 bg-gradient-to-br from-amber-950/80 via-black to-zinc-950 text-amber-500/30 shadow-black/80"
          : red
            ? "border-red-500/50 bg-gradient-to-b from-zinc-900 to-black text-red-500 shadow-red-950/30"
            : "border-zinc-300/40 bg-gradient-to-b from-zinc-800 to-black text-zinc-100 shadow-zinc-950/50",
      )}
    >
      {hidden || !card ? (
        ...
        <span className="font-mono tracking-tighter text-sm sm:text-base drop-shadow-md">{cardLabel(card)}</span>
      )}
    </motion.div>
  );
}
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-black/80 p-4 backdrop-blur-xl flex flex-wrap items-center justify-between gap-3">
          <div>
            <span className="font-mono text-[10px] uppercase tracking-widest text-amber-400">
              Buy-In ${Number(tournament.buy_in).toFixed(0)}
            </span>
            <h2 className="font-display text-2xl font-bold text-white tracking-wide">{tournament.name}</h2>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={onLeave}
              className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 font-mono text-xs text-zinc-300 hover:bg-white/10 transition"
            >
              ZPĚT DO LOBBY
            </button>

            {/* Tlačítko ZRUŠIT TURNAJ - vynuceně viditelné */}
            <button
              onClick={onCancel}
              className="inline-flex items-center gap-1.5 rounded-xl border border-red-500/40 bg-red-500/20 px-3 py-2 font-mono text-xs font-bold text-red-400 hover:bg-red-500/30 transition shadow-lg shadow-red-500/10"
            >
            ...
            {seated && (
              <button
                onClick={cashOut}
                className="inline-flex items-center gap-1.5 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2 font-mono text-xs font-bold text-amber-400 hover:bg-amber-500/20 transition"
              >
                <LogOut className="h-3.5 w-3.5" /> CASH OUT
              </button>
            )}

            {/* Tlačítko ROZDAT KARTY */}
            {(!hand || hand.stage === "done") && (
              <button
                onClick={deal}
                className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 px-4 py-2 font-mono text-xs font-black text-black hover:brightness-110 shadow-lg shadow-amber-500/20 transition"
              >
                <Play className="h-3.5 w-3.5 fill-black" /> ROZDAT KARTY
              </button>
            )}
          </div>
        </div>

        {/* NEON POKER TABLE FELT */}
        <div className="relative overflow-hidden rounded-[2.5rem] border-4 border-amber-950/60 bg-emerald-950 p-6 sm:p-10 shadow-[inset_0_0_80px_rgba(0,0,0,0.85)] ring-1 ring-amber-500/20">
          ...
          <div className="relative z-10 flex flex-col items-center justify-center gap-4 py-4">
            <span className="rounded-full border border-emerald-500/30 bg-emerald-950/80 px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-widest text-emerald-300 backdrop-blur-md shadow-lg">
            ...
            <div className="inline-flex items-center gap-2 rounded-2xl border border-amber-500/40 bg-black/80 px-5 py-2 font-mono text-sm font-black text-amber-400 shadow-2xl backdrop-blur-xl">
              <Coins className="h-5 w-5 text-amber-400 animate-bounce" /> POT: <CountUp value={hand?.pot ?? 0} />
            </div>
          </div>
            ...
            {seats.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  className={cn(
                    "relative overflow-hidden rounded-2xl border p-3 transition-all duration-300 backdrop-blur-md",
                    isTurn
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-display font-bold text-sm text-zinc-100">{s.nickname}</span>
                    {isTurn && (
                      <span className="inline-flex items-center gap-1 font-mono text-[10px] font-bold text-amber-400 animate-pulse">
                  ...
                  <div className="mt-1 flex items-center justify-between font-mono text-[11px] text-zinc-400">
                    <span>
                      Chips: <CountUp value={p?.chips ?? s.chips} className="text-amber-400 font-bold" />
                    </span>
                    {p && p.bet > 0 && <span className="text-emerald-400 font-bold">Bet: ${p.bet}</span>}
                    {p?.allIn && (
                      <span className="rounded bg-red-500/20 px-1 py-0.5 text-[9px] font-black text-red-400 border border-red-500/30">
                        ALL-IN
                      </span>
                    )}
                  </div>
                  ...
                  {winner && (
                    <div className="mt-2 text-center rounded-lg bg-emerald-500/20 py-1 font-mono text-[10px] font-black uppercase text-emerald-300 border border-emerald-500/30">
        ...
        {seated && hand && hand.stage !== "done" && (
          <div className="rounded-3xl border border-white/10 bg-black/90 p-4 backdrop-blur-xl flex flex-wrap items-center gap-3">
            <button
              disabled={!myTurn}
              onClick={() => act("fold")}
              className="rounded-xl border border-red-500/40 bg-red-500/10 px-4 py-2.5 font-mono text-xs font-black uppercase tracking-wider text-red-400 hover:bg-red-500/20 disabled:opacity-30 transition"
            >
              Fold
            </button>
            <button
              ...
              onClick={() => act("check")}
              className="rounded-xl border border-white/20 bg-white/5 px-4 py-2.5 font-mono text-xs font-black uppercase tracking-wider text-white hover:bg-white/10 disabled:opacity-30 transition"
            >
              Check
            </button>
            <button
              disabled={!myTurn || toCall === 0}
              onClick={() => act("call")}
              className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-2.5 font-mono text-xs font-black uppercase tracking-wider text-amber-300 hover:bg-amber-500/20 disabled:opacity-30 transition"
            >
              Call {toCall > 0 ? `$${toCall}` : ""}
            </button>

            <div className="flex min-w-[200px] flex-1 items-center gap-3 bg-white/5 p-2 rounded-xl border border-white/5">
              <input
                type="range"
              ...
              />
              <span className="w-16 font-mono text-xs font-bold text-amber-400 text-right">${raise}</span>
            </div>

            <button
              disabled={!myTurn}
              onClick={() => act("raise", raise)}
              className="rounded-xl bg-amber-500 px-4 py-2.5 font-mono text-xs font-black uppercase tracking-wider text-black hover:brightness-110 disabled:opacity-30 transition shadow-lg shadow-amber-500/20"
            >
              Raise
            </button>
            <button
              disabled={!myTurn}
              onClick={() => act("allin")}
              className="rounded-xl bg-gradient-to-r from-red-600 via-amber-600 to-red-600 px-4 py-2.5 font-mono text-xs font-black uppercase tracking-wider text-white hover:brightness-110 disabled:opacity-30 transition shadow-lg shadow-red-600/30"
            >
              All-In
            </button>
          </div>
        ...
        {hand && (
          <div className="rounded-2xl border border-white/5 bg-black/60 p-3 max-h-36 overflow-y-auto font-mono text-[11px] text-zinc-400 space-y-1">
            {hand.log.slice(-14).map((l, i) => (
              <p key={i} className="border-b border-white/5 pb-0.5 last:border-none">
                {l}
              </p>
            ))}
          </div>
