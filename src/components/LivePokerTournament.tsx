...
export function LivePokerTournament() {
  ...
  const cancel = async (id: string) => {
    ...
    if (error) return toast.error(mapError(error));
    toast.success(`Turnaj zrušen. Vráceno $${Number(data?.refunded ?? 0).toFixed(2)}.`);
    await refreshProfile();
    setOpenId(null);
    await load();
  };
...
function PokerTable({ tournament, seats, onBack, onRefresh, onCancel }: { tournament: Tournament; seats: Seat[]; onBack: ()=>void; onRefresh: ()=>Promise<void>|void; onCancel: ()=>void }) {
  ...
  const cashOut=async()=>{const res=await rpc("poker_cash_out",{_tournament_id:tournament.id});if(res){toast.success(`Vyplaceno $${Number(res.cashed??0).toFixed(2)}.`);await refreshProfile();onBack();}};
