...
import { ArrowRightLeft, Coins, Loader2, X } from "lucide-react";
import { toast } from "sonner";
import { EXCHANGE_RATE, useWallet } from "@/lib/wallet";

/** Směnárna: 1 Dollar = 100 Slot CZK. Dollar balance je uložený v Supabase. */
export function CurrencyExchangeModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function submit() {
    ...
    setSubmitting(true);
    setError(null);
    const res = dir === "toSlot" ? await exchangeToSlot(numeric) : await exchangeToDollars(numeric);
    setSubmitting(false);
    if (!res.ok) {
  ...
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          <motion.div
            initial={{ y: 24, scale: 0.96, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} exit={{ y: 16, opacity: 0 }}
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={onClose} aria-label="Zavřít" className="rounded-lg border border-hop-gold/30 p-1.5 text-hop-gold"><X className="h-4 w-4" /></button>
            </div>
            ...
            <button disabled={submitting} onClick={() => { setDir((d) => (d === "toSlot" ? "toDollars" : "toSlot")); setError(null); }}
              className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-hop-gold/30 bg-hop-950/70 px-3 py-2 text-[11px] font-black uppercase tracking-[0.18em] text-hop-gold disabled:opacity-50">
              <ArrowRightLeft className="h-4 w-4" /> {dir === "toSlot" ? "Dolary → Slot CZK" : "Slot CZK → Dolary"}
            </button>

            <label className="mt-3 block">
              <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-hop-neon/70">Částka ({dir === "toSlot" ? "$" : "Slot CZK"})</span>
            ...
            {error && <p className="mt-2 rounded-lg border border-rose-400/50 bg-rose-500/10 px-3 py-1.5 text-xs font-bold text-rose-300">{error}</p>}

            <button disabled={submitting} onClick={submit}
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border-2 border-hop-gold/70 bg-gradient-to-b from-hop-gold via-amber-500 to-hop-gold-deep px-4 py-3 font-display text-sm font-black uppercase tracking-[0.16em] text-hop-950 disabled:cursor-wait disabled:opacity-60">
              {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Coins className="h-4 w-4" />} {submitting ? "Zpracovávám…" : "Směnit"}
            </button>
          </motion.div>
        </motion.div>
      )}
