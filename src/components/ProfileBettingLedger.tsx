  ...
  return (
    <section className="mt-6 rounded-2xl border border-primary/20 bg-background/50 p-4 backdrop-blur">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-lg tracking-wider text-primary">BETTING LEDGER</h2>
          <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Oficiální payouty a refundy</p>
        </div>
        <Coins className="h-5 w-5 text-primary" />
      </div>
      {rows.length === 0 ? (
        <p className="py-5 text-sm text-muted-foreground">Zatím žádné vypořádané sázky.</p>
      ) : (
          ...
          {rows.map((row) => (
            <div key={row.id} className="flex items-center justify-between rounded-xl border border-border/60 bg-background/60 px-3 py-2">
              <div className="flex items-center gap-2">
                {row.kind === "bet_payout" ? <Trophy className="h-4 w-4 text-accent" /> : <RotateCcw className="h-4 w-4 text-primary" />}
                <div>
                  <p className="text-sm font-medium">{row.kind === "bet_payout" ? "Výhra ze sázky" : "Refund sázky"}</p>
                  <p className="text-[10px] text-muted-foreground">{new Date(row.created_at).toLocaleString("cs-CZ")}</p>
                </div>
              </div>
