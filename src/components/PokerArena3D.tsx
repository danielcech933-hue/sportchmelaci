    ...
    <div className="relative overflow-hidden rounded-[34px] border border-amber-400/20 bg-[#05070b] shadow-[0_40px_140px_-50px_rgba(255,196,67,.45)]">
      ...
      <header className="relative z-10 flex flex-col gap-4 border-b border-white/8 bg-black/35 px-5 py-5 backdrop-blur-xl sm:px-7 sm:py-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2 font-mono text-[8px] font-black uppercase tracking-[.16em]">
          ...
          <span className="inline-flex items-center gap-1 rounded-full border border-amber-400/25 bg-amber-400/10 px-3 py-1.5 text-amber-300"><Gem className="h-3 w-3" /> SLOT CZK</span>
        </div>
      </header>
      ...
      <section className="relative z-10 p-3 sm:p-5">
        <div className="relative overflow-hidden rounded-[30px] border border-amber-300/15 bg-gradient-to-b from-[#111824] via-[#071016] to-black p-3 sm:p-5 [perspective:1600px]">
          ...
          <div className="relative mx-auto min-h-[390px] max-w-6xl [transform-style:preserve-3d] sm:min-h-[470px]">
            <motion.div initial={{ rotateX: 58, scale: .96, opacity: 0 }} animate={{ rotateX: 58, scale: 1, opacity: 1 }} transition={{ duration: .9, ease: "easeOut" }} className="absolute left-1/2 top-[48%] h-[66%] w-[88%] -translate-x-1/2 -translate-y-1/2 rounded-[50%] border-[3px] border-amber-300/25 bg-[radial-gradient(ellipse_at_center,rgba(20,76,53,.95),rgba(2,11,12,.98)_58%,rgba(0,0,0,.99))] shadow-[0_45px_100px_-35px_rgba(0,0,0,.95),inset_0_0_0_10px_rgba(30,28,15,.55),inset_0_0_60px_rgba(0,0,0,.9)] [transform-style:preserve-3d]">
              ...
              <div className="absolute left-1/2 top-1/2 h-40 w-40 -translate-x-1/2 -translate-y-1/2 rounded-full border border-amber-300/20 bg-black/20 shadow-[0_0_90px_rgba(255,204,68,.09)]" />
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                <div className="font-mono text-[8px] font-black uppercase tracking-[.4em] text-white/25">CHMEL ARENA</div>
                <div className="mt-1 font-display text-2xl tracking-[.25em] text-amber-200/70">NO LIMIT</div>
              </div>
            </motion.div>
            ...
            {SEATS.map((seat, i) => {
              ...
              const y = 48 + Math.sin((angle * Math.PI) / 180) * 31;
              return (
                <motion.div key={seat.name} initial={{ opacity: 0, scale: .8, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ delay: .15 + i * .08, duration: .45 }} className="absolute z-20 w-[150px] -translate-x-1/2 -translate-y-1/2 sm:w-[180px]" style={{ left: `${x}%`, top: `${y}%` }}>
                  <div className={`relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br ${seat.tone} bg-[#070a0f]/90 p-2.5 shadow-[0_22px_45px_-26px_rgba(0,0,0,.95)] backdrop-blur-xl`}>
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex min-w-0 items-center gap-2"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-xl border border-white/10 bg-black/30 text-white/70"><Users className="h-4 w-4" /></div><div className="min-w-0"><div className="truncate font-display text-[12px] tracking-wider text-white">{seat.name}</div><div className="font-mono text-[7px] uppercase tracking-[.18em] text-white/35">{seat.meta}</div></div></div>
                      {i === 0 && <Crown className="h-4 w-4 text-amber-300" />}
                    </div>
                    <div className="mt-2 flex items-center justify-between rounded-lg border border-white/8 bg-black/30 px-2 py-1.5 font-mono text-[8px] uppercase tracking-[.12em]"><span className="text-white/35">STACK</span><span className="font-black text-amber-200">1 000</span></div>
                    <div className="mt-1.5 flex gap-1.5"><span className="grid h-7 w-5 place-items-center rounded-md border border-white/15 bg-gradient-to-b from-slate-700 to-black text-[7px] text-white/40">?</span><span className="grid h-7 w-5 place-items-center rounded-md border border-white/15 bg-gradient-to-b from-slate-700 to-black text-[7px] text-white/40">?</span><span className="ml-auto rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2 py-1 font-mono text-[7px] font-black uppercase text-emerald-300">ONLINE</span></div>
                  </div>
                </motion.div>
              );
            })}
            ...
            <div className="absolute left-1/2 top-[28%] z-30 -translate-x-1/2">
              <div className="flex items-center gap-2 rounded-2xl border border-amber-300/25 bg-black/65 px-3 py-2 font-mono text-[8px] font-black uppercase tracking-[.18em] text-amber-200 shadow-xl backdrop-blur-xl"><Zap className="h-4 w-4" /> ACTION TIMER 00:25</div>
            </div>
          </div>
        </div>
      </section>
