...
export const Route = createFileRoute("/rankings")({
  head: () => ({
    meta: [
      { title: "Žebříček 2.0 — Chmeloví Sportovci" },
      { name: "description", content: "Oficiální sportovní žebříček s ELO, formou, sériemi a pořadím jednotlivých sportů." },
    ],
  }),
...
type Mode = "solo" | "team";
const PODIUM = [
  { img: goldImg, label: "ŠAMPION", ring: "rank-gold" },
  { img: silverImg, label: "2. MÍSTO", ring: "rank-silver" },
  { img: bronzeImg, label: "3. MÍSTO", ring: "rank-bronze" },
];
    ...
    <main className="relative mx-auto max-w-[1450px] px-3 pb-32 pt-4 sm:px-5 lg:px-7">
      ...
      <section className="relative overflow-hidden rounded-[32px] border border-amber-300/20 bg-[linear-gradient(135deg,rgba(12,15,19,.98),rgba(1,4,7,.99))] shadow-[0_35px_120px_-55px_rgba(250,204,21,.55)]">
        ...
        <div className="relative grid lg:grid-cols-[1.15fr_.85fr]">
          <div className="p-6 sm:p-9 lg:p-12">
            <div className="flex flex-wrap items-center gap-2"><span className="aaa-meta text-amber-200/80">CHMELOVÍ SPORTOVCI · KOMPETITIVNÍ SYSTÉM</span><span className="rounded-full border border-emerald-300/20 bg-emerald-300/5 px-2.5 py-1 font-mono text-[8px] font-black uppercase tracking-[.2em] text-emerald-200">ŽEBŘÍČEK ŽIVĚ</span></div>
            <h1 className="mt-5 font-display text-6xl font-black tracking-[.08em] text-white sm:text-7xl lg:text-8xl">ŽEBŘÍ<span className="gold-text">ČEK</span></h1>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-white/45 sm:text-base">Oficiální ELO, výhry a prohry, aktuální série a pořadí pro 1v1 i 2v2. Jeden přehled pro celý SportChmeláci systém.</p>
            <div className="mt-7 flex flex-wrap gap-2"><Link to="/sport-center" className="aaa-ghost inline-flex items-center gap-2 px-5 py-3 text-[10px] font-black uppercase tracking-[.16em]"><Radio className="h-4 w-4" /> Sport centrum</Link><Link to="/trophy-room" className="aaa-cta inline-flex items-center gap-2 px-5 py-3 text-[10px] font-black uppercase tracking-[.16em]"><Trophy className="h-4 w-4" /> Síň trofejí</Link></div>
          </div>
          <div className="grid grid-cols-2 border-t border-white/8 bg-black/25 sm:grid-cols-4 lg:grid-cols-2 lg:border-l lg:border-t-0">
            <Metric label="HRÁČI" value={loading ? "—" : globalStats.players} sub="v tabulce" icon={<Users className="h-4 w-4" />} />
            <Metric label="ZÁPASY" value={loading ? "—" : globalStats.matches} sub="rozhodnuté" icon={<BarChart3 className="h-4 w-4" />} />
            <Metric label="ŽIVĚ" value={globalStats.live} sub="otevřené" icon={<Zap className="h-4 w-4" />} />
            <Metric label="PRŮM. ELO" value={globalStats.avgElo || "—"} sub={`${globalStats.avgWinRate}% úspěšnost`} icon={<Crown className="h-4 w-4" />} />
          </div>
        </div>
      </section>
      ...
      <section className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/8 bg-black/20 p-3">
        <div className="flex flex-wrap gap-1.5">
          {(["solo", "team"] as Mode[]).map((m) => <button key={m} onClick={() => setMode(m)} className={`rounded-xl px-4 py-2.5 text-[9px] font-black uppercase tracking-[.18em] transition ${mode === m ? "bg-amber-300 text-black shadow-[0_0_25px_-12px_rgba(250,204,21,.9)]" : "text-white/35 hover:bg-white/[.03] hover:text-white"}`}>{m === "solo" ? "JEDNOTLIVCI" : "2V2 TÝMY"}</button>)}
        </div>
      ...
      {err && <div className="mt-3"><StateBlock state="error" title="Žebříček se nepodařilo načíst" hint={err} /></div>}

      <section className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <SignalCard label="KRÁL SÉRIE" value={streakLeader?.row.label ?? "—"} hint={streakLeader?.streak ? `${streakLeader.streak} výher v řadě` : "Žádná aktivní série"} icon={<Flame className="h-4 w-4" />} />
        <SignalCard label="NEJLEPŠÍ FORMA" value={formLeader?.label ?? "—"} hint={formLeader ? `${Math.round(winRate(formLeader) * 100)}% úspěšnost` : "Čeká se"} icon={<Sparkles className="h-4 w-4" />} />
        <SignalCard label="TÝMOVÝ SIGNÁL" value={globalStats.twoVtwo ? `${globalStats.twoVtwo}` : "PŘIPRAVENO"} hint={globalStats.twoVtwo ? "2v2 zápasů" : "Zatím bez týmových zápasů"} icon={<Shield className="h-4 w-4" />} />
        <SignalCard label="STAV ŽEBŘÍČKU" value={loading ? "SYNCHRONIZACE" : "ŽIVĚ"} hint="aktualizace každých 5 sekund" icon={<Radio className="h-4 w-4" />} />
      </section>
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[1.2fr_.8fr]">
        <div className="rounded-[26px] border border-white/8 bg-black/20 p-5 sm:p-6">
          <div className="flex items-center justify-between gap-3"><div><div className="aaa-meta">STUPNĚ VÍTĚZŮ</div><h2 className="mt-1 font-display text-2xl tracking-wider text-white">TOP 3</h2></div><div className="font-mono text-[9px] uppercase tracking-[.2em] text-white/25">{mode === "solo" ? "JEDNOTLIVCI" : "TÝMY"} · {sport === "all" ? "VŠECHNY SPORTY" : SPORTS[sport].name}</div></div>
        ...
        <div className="rounded-[26px] border border-white/8 bg-black/20 p-5 sm:p-6">
          <div className="aaa-meta">LÍDŘI SPORTŮ</div><h2 className="mt-1 font-display text-2xl tracking-wider text-white">KRÁLOVÉ ARÉN</h2>
          <div className="mt-4 space-y-2">{sportLeaders.slice(0, 6).map((s) => <button key={s.id} onClick={() => setSport(s.id)} className="flex w-full items-center justify-between rounded-2xl border border-white/7 bg-white/[.02] p-3 text-left hover:border-cyan-300/20"><div className="flex min-w-0 items-center gap-3"><span className="text-xl">{s.emoji}</span><div className="min-w-0"><div className="truncate font-display text-sm text-white">{s.name}</div><div className="aaa-meta mt-1">{s.row?.label ?? "BEZ DAT"}</div></div></div><div className="font-mono text-[9px] text-amber-200">{s.row?.wins ?? 0} V</div></button>)}</div>
        </div>
      </section>

      <section className="mt-4 overflow-hidden rounded-[26px] border border-white/8 bg-black/20">
        <div className="flex items-center justify-between gap-4 border-b border-white/8 p-5 sm:p-6"><div><div className="aaa-meta">OFICIÁLNÍ TABULKA</div><h2 className="mt-1 font-display text-2xl tracking-wider text-white">KONKURENČNÍ POŘADÍ</h2></div><div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/25">ELO · V · P · ÚSPĚŠNOST</div></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b border-white/8 text-[9px] uppercase tracking-[.18em] text-white/25"><th className="px-5 py-3 text-left">#</th><th className="px-5 py-3 text-left">HRÁČ / TÝM</th><th className="px-5 py-3 text-right">ELO</th><th className="px-5 py-3 text-right">V</th><th className="px-5 py-3 text-right">P</th><th className="px-5 py-3 text-right">FORMA</th><th className="px-5 py-3 text-right">ÚSPĚŠNOST</th></tr></thead><tbody>{rest.map((r, i) => <RankingRow key={r.key} row={r} rank={i + 4} avatarPath={avatarByNick.get(r.key)} />)}{rows.length === 0 && <tr><td colSpan={7}><EmptyState /></td></tr>}</tbody></table></div>
      </section>

      <div className="mt-4 grid gap-4 md:grid-cols-3"><Link to="/leagues" className="rounded-2xl border border-amber-300/15 bg-amber-300/[.03] p-5 hover:border-amber-300/30"><div className="aaa-meta">SEZÓNA</div><div className="mt-2 font-display text-xl text-white">CHMEL LIGA</div><div className="mt-1 text-sm text-white/30">Body, matchday a dlouhodobá forma.</div></Link><Link to="/records" className="rounded-2xl border border-cyan-300/15 bg-cyan-300/[.03] p-5 hover:border-cyan-300/30"><div className="aaa-meta">SÍŇ SLÁVY</div><div className="mt-2 font-display text-xl text-white">REKORDNÍ RADAR</div><div className="mt-1 text-sm text-white/30">Největší série a historické výkony.</div></Link><Link to="/activity" className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[.03] p-5 hover:border-emerald-300/30"><div className="aaa-meta">ŽIVÝ SIGNÁL</div><div className="mt-2 font-display text-xl text-white">CO SE DĚJE</div><div className="mt-1 text-sm text-white/30">Co se právě děje ve SportChmelácích.</div></Link></div>
    </main>
  );
}
