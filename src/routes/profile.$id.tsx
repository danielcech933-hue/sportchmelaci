  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#02050a] pb-28 pt-3 text-white">
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute left-1/2 top-0 h-[720px] w-[1050px] -translate-x-1/2 rounded-full bg-[radial-gradient(circle,rgba(250,204,21,.10),transparent_64%)] blur-3xl" />
        <div className="absolute right-[-150px] top-[28%] h-[580px] w-[580px] rounded-full bg-[radial-gradient(circle,rgba(34,211,238,.065),transparent_64%)] blur-3xl" />
      </div>
      ...
      <div className="mx-auto max-w-[1380px] px-3 sm:px-5 lg:px-7">
        <section className="relative mb-4 overflow-hidden rounded-[32px] border border-white/10 bg-[linear-gradient(135deg,rgba(13,17,22,.97),rgba(2,5,9,.995))] shadow-[0_35px_120px_-65px_rgba(250,204,21,.55)]">
          <div className="absolute inset-0 opacity-35 [background-image:linear-gradient(rgba(255,255,255,.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.025)_1px,transparent_1px)] [background-size:42px_42px]" />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_10%_0%,rgba(250,204,21,.15),transparent_22%),radial-gradient(circle_at_90%_100%,rgba(34,211,238,.08),transparent_25%)]" />
          <div className="relative grid gap-5 p-5 sm:p-7 lg:grid-cols-[1fr_auto] lg:items-end lg:p-8">
            <div>
              <div className="flex flex-wrap items-center gap-2"><span className="aaa-meta text-amber-200/75">SPORTCHMELÁCI · PLAYER CARD</span><span className="rounded-full border border-emerald-300/20 bg-emerald-300/5 px-2.5 py-1 font-mono text-[8px] font-black uppercase tracking-[.2em] text-emerald-200">VEŘEJNÝ PROFIL</span></div>
              <h1 className="mt-3 font-display text-4xl font-black tracking-[.08em] text-white sm:text-6xl">PLAYER <span className="gold-text">PROFILE</span></h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-white/42">Výsledky, forma, trofeje, fotky a komunitní aktivita jednoho hráče v jednom prémiovém profilu.</p>
            </div>
            <div className="grid grid-cols-2 gap-2 sm:min-w-[280px]">
              <MiniSignal icon={<Crown className="h-4 w-4" />} label="REŽIM" value={isSelf ? "TVŮJ PROFIL" : "PLAYER"} />
              <MiniSignal icon={<ShieldCheck className="h-4 w-4" />} label="PŘÍSTUP" value={user ? "AUTH" : "PUBLIC"} />
            </div>
          </div>
        </section>
        ...
        <section id="activity" className="mt-5 scroll-mt-28">
          <div className="mb-3 flex items-center gap-2 text-amber-200/65"><Sparkles className="h-4 w-4" /><span className="aaa-meta">COMMUNITY SIGNAL</span></div>
          <SocialHub profileUserId={id} />
        </section>
...
function MiniSignal({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="rounded-2xl border border-white/8 bg-white/[.025] p-3.5"><div className="flex items-center gap-2 text-cyan-200/65">{icon}<span className="aaa-meta">{label}</span></div><div className="mt-2 truncate font-display text-lg tracking-[.08em] text-white">{value}</div></div>;
}
