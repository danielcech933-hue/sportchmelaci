...
function MatchCard({m,status}:{m:Match;status:string}) { const cfg=SPORTS[m.sport]; const pool=betsPool(m.bets); const nB=uniqueBettors(m.bets); const totals={a:m.bets.filter(b=>b.pick==='a').reduce((s,b)=>s+(b.amount??0),0),b:m.bets.filter(b=>b.pick==='b').reduce((s,b)=>s+(b.amount??0),0)}; const pctA=pool?(totals.a/pool)*100:50; return <Link to="/match" search={{id:m.id}} className="panel neon-border block max-w-full overflow-hidden p-3 transition hover:brightness-110 sm:p-4"><div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-1 text-[10px] font-mono uppercase tracking-[0.2em] sm:tracking-[0.3em]"><span className="min-w-0 truncate text-primary">{cfg.emoji} {cfg.name}</span><span className="shrink-0 text-accent">{status} · {nB}</span></div><div className="mt-2 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-baseline gap-2"><span className="min-w-0 truncate font-display text-base sm:text-lg">{m.teamA}</span><span className="shrink-0 font-mono text-xl text-primary sm:text-2xl">{m.scoreA} : {m.scoreB}</span><span className="min-w-0 truncate text-right font-display text-base sm:text-lg">{m.teamB}</span></div><div className="mt-3 flex h-2 overflow-hidden rounded-full bg-background/60"><div className="bg-primary" style={{width:`${pctA}%`}}/><div className="bg-accent" style={{width:`${100-pctA}%`}}/></div><div className="mt-1 flex justify-between font-mono text-[10px] text-muted-foreground"><span>${totals.a}</span><span className="text-accent">Pool ${pool}</span><span>${totals.b}</span></div></Link>; }
...
function SettledCard({m}:{m:Match}){const cfg=SPORTS[m.sport]; const winners=m.bets.filter(b=>b.status==='won'); const refunded=m.bets.some(b=>b.status==='refunded'); return <Link to="/match" search={{id:m.id}} className="panel block max-w-full overflow-hidden p-3 opacity-90 transition hover:brightness-110 sm:p-4"><div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-1 text-[10px] font-mono uppercase tracking-[0.2em] sm:tracking-[0.3em]"><span className="min-w-0 truncate text-muted-foreground">{cfg.emoji} {cfg.name}</span><span className="shrink-0 text-accent">{refunded?"REFUNDED":"SETTLED"}</span></div><div className="mt-2 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-baseline gap-2"><span className="min-w-0 truncate font-display">{m.teamA}</span><span className="shrink-0 font-mono text-primary">{m.scoreA} : {m.scoreB}</span><span className="min-w-0 truncate text-right font-display">{m.teamB}</span></div>{winners.length>0&&<div className="mt-2 flex flex-wrap gap-1 text-[10px]">{winners.map(b=><span key={b.id} className="max-w-full truncate rounded bg-accent/20 px-1.5 py-0.5 font-mono text-accent">{b.bettor} +${b.payout?.toFixed(2)??0} @ {b.lockedOdds?.toFixed(2)??"—"}</span>)}</div>}</Link>; }
