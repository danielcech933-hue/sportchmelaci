...
function UltimateTeamPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 pb-28 pt-6">
      <header className="rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/10 via-background/70 to-background/40 p-4 shadow-[0_18px_60px_-35px_hsl(var(--primary)/0.55)] sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 font-display text-3xl uppercase tracking-[0.12em] text-primary sm:text-4xl">
              {club?.clubName ?? "Můj klub"}
            </h1>
            <p className="mt-2 max-w-xl text-sm text-muted-foreground">
              Tvoje karty, mince, Card Spin a serverově uložená sestava na jednom místě.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <button
              ...
              onClick={() => void reload()}
              disabled={refreshing}
              title="Obnovit klub a sbírku ze serveru"
              className="inline-flex items-center gap-2 rounded-lg border border-border/60 bg-background/60 px-3 py-1.5 font-mono text-[10px] uppercase tracking-widest text-muted-foreground transition hover:border-primary/40 hover:text-primary disabled:opacity-50"
            >
              <RefreshCw className={cn("h-3.5 w-3.5", refreshing && "animate-spin")} />
              {refreshing ? "Obnovuji" : "Synchronizovat"}
            </button>
          </div>
        </div>
      </header>
      ...
      {error && (
        <div className="mt-3 flex items-center justify-between gap-3 rounded-xl border border-danger/30 bg-danger/5 px-3 py-2 text-xs text-danger">
          <span>{error}</span>
          <button type="button" onClick={() => void reload()} className="font-mono uppercase tracking-widest underline underline-offset-4">Zkusit znovu</button>
        </div>
      )}
      ...
      <section className="mt-5">
        {tab === "club" && (
          <div className="space-y-4">
            ...
            <div className="rounded-3xl border border-primary/20 bg-background/50 p-4 sm:p-5">
              <div className="flex items-end justify-between gap-3">
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-primary/80">Klubová sbírka</p>
                  <h2 className="mt-1 font-display text-xl uppercase tracking-[0.08em]">Nejlepší hráči</h2>
                </div>
                <button type="button" onClick={() => setTab("collection")} className="font-mono text-[10px] uppercase tracking-widest text-primary hover:underline">Otevřít sbírku</button>
              </div>
              <div className="mt-4 flex gap-3 overflow-x-auto pb-2">
                {!topCards.length && <p className="text-sm text-muted-foreground">Zatím prázdno — začni Card Spinem.</p>}
                {topCards.map((c) => (
                  <button key={c.id} type="button" onClick={() => setTab("collection")} className="shrink-0 rounded-2xl border border-border/60 bg-background/60 p-3 text-left transition hover:-translate-y-0.5 hover:border-primary/40">
                    <span className="font-display text-2xl text-primary">{c.card.rating}</span>
                    <span className="ml-2 font-mono text-[10px] uppercase text-muted-foreground">{c.card.position}</span>
                    <p className="mt-1 max-w-[120px] truncate text-sm font-medium">{c.card.name}</p>
                    <p className="mt-0.5 max-w-[120px] truncate text-[10px] text-muted-foreground">{c.card.club}</p>
                  </button>
                ))}
              </div>
            </div>
