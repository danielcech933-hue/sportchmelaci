...
import { MatchCenterPulse } from "@/components/MatchCenterPulse";
...
function MatchPage() {
  ...
  return <main className="relative mx-auto max-w-[1450px] px-3 pb-28 pt-4 sm:px-5 lg:px-7">
    ...
    <section className="relative overflow-hidden rounded-[30px] border border-amber-300/20 bg-[linear-gradient(135deg,rgba(15,18,22,.98),rgba(3,6,9,.99))] shadow-[0_35px_120px_-55px_rgba(250,204,21,.55)]">
      <div className="absolute inset-0 opacity-45 [background-image:linear-gradient(rgba(255,255,255,.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.025)_1px,transparent_1px)] [background-size:44px_44px]" /><div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_18%,rgba(250,204,21,.15),transparent_28%)]" />
      <div className="relative grid gap-0 xl:grid-cols-[1fr_320px]"><div className="p-5 sm:p-8 lg:p-10">
        ...
        <div className="mt-6 grid w-full max-w-5xl grid-cols-[1fr_auto_1fr] items-center gap-3 self-center sm:mt-10 sm:gap-8"><TeamHero align="right" title={match.teamA} players={teamAPlayers} accent="gold" /><div className="text-center"><div className="font-mono text-[9px] font-black uppercase tracking-[.35em] text-white/25">{ended ? "FINAL" : "LIVE"}</div><div className="mt-2 font-display text-4xl font-black tracking-[.08em] text-white sm:text-6xl md:text-7xl">{match.scoreA}<span className="mx-2 text-white/15 sm:mx-4">:</span>{match.scoreB}</div>{cfg.hasSets && <div className="mt-2 font-mono text-[9px] uppercase tracking-[.25em] text-amber-200/55">{cfg.setLabel.toUpperCase()} {setsA}:{setsB}</div>}</div><TeamHero title={match.teamB} players={teamBPlayers} accent="cyan" /></div>
        {cfg.hasSets && <div className="mt-6 flex flex-wrap items-center justify-center gap-2"><span className="aaa-meta">SET HISTORY</span>{match.sets.map((s,i)=><span key={i} className="rounded-lg border border-white/10 bg-white/[.03] px-3 py-1.5 font-mono text-[10px] text-white/55">{s.a}–{s.b}</span>)}{match.sets.length===0&&<span className="text-[10px] text-white/20">No completed sets</span>}</div>}
      </div><aside className="border-t border-white/8 bg-black/20 p-5 sm:p-7 xl:border-l xl:border-t-0"><div className="aaa-meta text-cyan-200/70">LIVE CONTROL DECK</div><div className="mt-5 grid grid-cols-2 gap-2"><DeckStat label="STATUS" value={ended?"ENDED":"LIVE"} icon={<Radio className="h-3.5 w-3.5" />} live={!ended}/><DeckStat label="TOTAL SCORE" value={String(currentTotal)} icon={<Gauge className="h-3.5 w-3.5" />} /></div><div className="mt-3 rounded-2xl border border-amber-300/12 bg-amber-300/[.03] p-4"><div className="aaa-meta">FORM SIGNAL</div><div className="mt-3 flex gap-1.5">{form.length?form.map((x,i)=><span key={i} className={`grid h-7 w-7 place-items-center rounded-lg border text-[9px] font-black ${x==="W"?"border-emerald-300/25 bg-emerald-300/10 text-emerald-200":x==="L"?"border-rose-300/25 bg-rose-300/10 text-rose-200":"border-white/10 bg-white/[.03] text-white/35"}`}>{x}</span>):<span className="text-[9px] text-white/20">NO FORM DATA</span>}</div></div><div className="mt-3 rounded-2xl border border-white/8 bg-white/[.02] p-4"><div className="aaa-meta">MATCH SIGNAL</div><div className="mt-3 space-y-2"><Signal icon={<Swords className="h-3.5 w-3.5" />} label="FORMAT" value={match.matchFormat === "2v2" ? "TEAM BATTLE" : "SOLO DUEL"} /><Signal icon={<Sparkles className="h-3.5 w-3.5" />} label="SPORT" value={cfg.name.toUpperCase()} /><Signal icon={<CalendarClock className="h-3.5 w-3.5" />} label="STARTED" value={new Date(match.startedAt).toLocaleString("cs-CZ", { day:"2-digit", month:"2-digit", hour:"2-digit", minute:"2-digit" })} /></div></div>{isOwner&&<Link to="/live" search={{id:match.id}} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-amber-300 px-4 py-3 text-[10px] font-black uppercase tracking-[.16em] text-black shadow-[0_0_28px_-12px_rgba(250,204,21,.9)]"><Radio className="h-4 w-4"/> Live rozhodčí</Link>}</aside></div>
    </section>

    <MatchCenterPulse match={match} />

    <section className="mt-4 grid gap-4 xl:grid-cols-[1fr_1fr]"><div className="rounded-[24px] border border-white/8 bg-black/20 p-4 sm:p-5"><SectionHead icon={<Users className="h-4 w-4"/>} kicker="LINEUPS" title="TÝMY & HRÁČI" />{match.matchFormat==="2v2"?<div className="mt-4 grid gap-3 sm:grid-cols-2"><LineupCard label="TEAM A" players={teamAPlayers} accent="gold"/><LineupCard label="TEAM B" players={teamBPlayers} accent="cyan"/></div>:<div className="mt-4 grid gap-3 sm:grid-cols-2"><LineupCard label="PLAYER A" players={teamAPlayers} accent="gold"/><LineupCard label="PLAYER B" players={teamBPlayers} accent="cyan"/></div>}{isAdmin&&!ended&&<><NicknamesDatalist options={nicknames}/><LineupEditor teamA={match.teamA} teamB={match.teamB} onSave={(a,b)=>update({...match,teamA:a,teamB:b,teamAPlayers:splitPlayers(a),teamBPlayers:splitPlayers(b)})}/></>}</div><div className="rounded-[24px] border border-white/8 bg-black/20 p-4 sm:p-5"><SectionHead icon={<Swords className="h-4 w-4"/>} kicker="HEAD TO HEAD" title="H2H ARCHIVE" />{h2h.length?<div className="mt-4 space-y-2">{h2h.map((m)=><Link key={m.id} to="/match" search={{id:m.id}} className="group flex items-center justify-between rounded-xl border border-white/8 bg-white/[.02] px-3 py-2.5 hover:border-amber-300/20"><div className="min-w-0"><div className="truncate font-display text-sm text-white/75">{m.teamA} <span className="text-white/20">vs</span> {m.teamB}</div><div className="aaa-meta mt-1">{new Date(m.endedAt!).toLocaleDateString("cs-CZ")}</div></div><span className="font-mono text-sm font-black text-amber-200">{m.scoreA}:{m.scoreB}</span><ChevronRight className="h-3.5 w-3.5 text-white/20 transition group-hover:text-amber-200"/></Link>)}</div>:<Empty text="Žádné předchozí přímé střety v této disciplíně."/>}</div></section>

    <section className="mt-4 grid gap-4 xl:grid-cols-[1.1fr_.9fr]"><div className="rounded-[24px] border border-white/8 bg-black/20 p-4 sm:p-5"><SectionHead icon={<BarChart3 className="h-4 w-4"/>} kicker="MATCH TELEMETRY" title="MOMENTUM"/><div className="mt-5 h-44 rounded-2xl border border-white/8 bg-black/30 p-4"><div className="flex h-full items-end gap-2">{buildMomentum(match,18).map((h,i)=><div key={i} className="group relative flex-1"><div className={`${i%2===0?"bg-amber-300/75":"bg-cyan-300/60"} mx-auto rounded-t-md transition-all duration-300 group-hover:brightness-125`} style={{height:`${h}%`}}/></div>)}</div></div><div className="mt-3 flex justify-between font-mono text-[8px] uppercase tracking-[.2em] text-white/20"><span>START</span><span>LIVE FLOW</span><span>{ended?"FINAL":"NOW"}</span></div></div><div className="rounded-[24px] border border-white/8 bg-black/20 p-4 sm:p-5"><SectionHead icon={<Trophy className="h-4 w-4"/>} kicker="MATCH TELEMETRY" title="KEY NUMBERS"/><div className="mt-4 grid grid-cols-2 gap-2"><NumberCard label="A SCORE" value={match.scoreA} tone="gold"/><NumberCard label="B SCORE" value={match.scoreB} tone="cyan"/><NumberCard label="A SETS" value={setsA} tone="gold"/><NumberCard label="B SETS" value={setsB} tone="cyan"/></div></div></section>

    {actionError&&<div className="mt-4 rounded-2xl border border-rose-300/20 bg-rose-300/5 p-4 text-sm text-rose-200">{actionError}</div>}{!isOwner&&<div className="mt-4 rounded-2xl border border-white/8 bg-white/[.02] p-3 text-center text-[10px] font-mono uppercase tracking-[.2em] text-white/30">Spectator mode · score controls are available to the match owner.</div>}

    {isOwner&&<section className="mt-4 rounded-[24px] border border-white/8 bg-black/20 p-4 sm:p-5"><SectionHead icon={<Zap className="h-4 w-4"/>} kicker="LIVE OPERATIONS" title="SCORE CONTROL"/><div className="mt-4 grid grid-cols-2 gap-3">{(["a","b"] as const).map(side=>{const score=side==="a"?match.scoreA:match.scoreB;const name=side==="a"?match.teamA:match.teamB;return <div key={side} className="rounded-2xl border border-white/8 bg-white/[.02] p-4"><div className="truncate text-center font-display text-lg text-white/70">{name}</div><div className="mt-3 font-display text-6xl text-center text-white">{score}</div><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={()=>bump(side,-1)} disabled={ended} className="rounded-xl border border-white/10 py-3 text-xl text-white/40 disabled:opacity-30">−</button><button onClick={()=>bump(side,1)} disabled={ended} className="rounded-xl bg-amber-300 py-3 text-2xl font-black text-black disabled:opacity-30">+1</button></div></div>})}</div><div className="mt-4 flex flex-wrap gap-2">{cfg.hasSets&&!ended&&<button onClick={finishSet} className="rounded-xl border border-cyan-300/20 bg-cyan-300/5 px-4 py-2.5 text-[10px] font-black uppercase tracking-[.15em] text-cyan-200">Ukončit {cfg.setLabel}</button>}{!ended&&<button onClick={resetScore} className="rounded-xl border border-white/10 bg-white/[.03] px-4 py-2.5 text-[10px] font-black uppercase tracking-[.15em] text-white/55">Reset score</button>}{!ended&&<button onClick={finishMatch} disabled={finishBusy} className="rounded-xl bg-amber-300 px-4 py-2.5 text-[10px] font-black uppercase tracking-[.15em] text-black disabled:opacity-50">{finishBusy?"Ukončuji…":"Finish match"}</button>}<button onClick={remove} className="rounded-xl border border-rose-300/15 bg-rose-300/5 px-4 py-2.5 text-[10px] font-black uppercase tracking-[.15em] text-rose-200/70">Delete</button></div></section>}

    <section className="mt-4 rounded-[24px] border border-amber-300/12 bg-[linear-gradient(180deg,rgba(250,204,21,.025),rgba(0,0,0,.15))] p-4 sm:p-5"><SectionHead icon={<Zap className="h-4 w-4"/>} kicker="LIVE BETTING" title="BETTING DESK"/><div className="mt-4"><BettingModule match={match} onRefresh={async()=>{const m=await fetchMatch(match.id);if(m)setMatch(m)}}/></div></section>
...
function TeamHero({ title, players, align = "left", accent }: { title:string; players:string[]; align?:"left"|"right"; accent:"gold"|"cyan" }){return <div className={`min-w-0 ${align==="right"?"text-right":"text-left"}`}><div className={`font-mono text-[9px] font-black uppercase tracking-[.28em] ${accent==="gold"?"text-amber-200/55":"text-cyan-200/55"}`}>{accent==="gold"?"TEAM A":"TEAM B"}</div><div className="mt-2 truncate font-display text-xl tracking-[.08em] text-white sm:text-3xl">{title}</div><div className="mt-2 flex flex-wrap gap-1.5" style={{justifyContent:align==="right"?"flex-end":"flex-start"}}>{players.map((p,i)=><span key={`${p}-${i}`} className="rounded-lg border border-white/8 bg-white/[.03] px-2 py-1 text-[9px] text-white/35">{p}</span>)}</div></div>}
function LineupCard({label,players,accent}:{label:string;players:string[];accent:"gold"|"cyan"}){return <div className="rounded-2xl border border-white/8 bg-white/[.02] p-4"><div className={`font-mono text-[8px] font-black uppercase tracking-[.24em] ${accent==="gold"?"text-amber-200/60":"text-cyan-200/60"}`}>{label}</div><div className="mt-3 space-y-2">{players.filter(Boolean).map((p,i)=><div key={`${p}-${i}`} className="flex items-center gap-2 rounded-xl border border-white/8 bg-black/20 px-3 py-2"><div className={`h-2 w-2 rounded-full ${accent==="gold"?"bg-amber-300":"bg-cyan-300"}`}/><span className="truncate font-display text-sm text-white/70"><NickLink nickname={p}/></span></div>)}</div></div>}
function DeckStat({label,value,icon,live}:{label:string;value:string;icon:ReactNode;live?:boolean}){return <div className="rounded-2xl border border-white/8 bg-white/[.025] p-3"><div className="flex items-center justify-between"><span className="aaa-meta">{label}</span><span className={live?"text-emerald-300":"text-amber-200/70"}>{icon}</span></div><div className={`mt-2 font-display text-lg tracking-[.1em] ${live?"text-emerald-200":"text-white"}`}>{value}</div></div>}
function Signal({icon,label,value}:{icon:ReactNode;label:string;value:string}){return <div className="flex items-center gap-3 rounded-xl border border-white/8 bg-black/20 px-3 py-2"><div className="text-amber-200/60">{icon}</div><div className="min-w-0 flex-1"><div className="aaa-meta">{label}</div><div className="truncate text-[11px] font-semibold text-white/60">{value}</div></div></div>}
...
function SectionHead({icon,kicker,title}:{icon:ReactNode;kicker:string;title:string}){return <div className="flex items-end justify-between gap-3"><div><div className="flex items-center gap-2 text-white/20"><span>{icon}</span><span className="aaa-meta">{kicker}</span></div><h2 className="mt-1 font-display text-2xl tracking-[.12em] text-white">{title}</h2></div><span className="font-mono text-[8px] uppercase tracking-[.22em] text-white/15">ULTRA S+</span></div>}
...
function LineupEditor({teamA,teamB,onSave}:{teamA:string;teamB:string;onSave:(a:string,b:string)=>void}){const [a,setA]=useState(teamA),[b,setB]=useState(teamB);return <div className="mt-4 grid grid-cols-2 gap-3"><div><label className="aaa-meta">ADMIN TEAM A</label><input value={a} list={NICKNAMES_DATALIST_ID} onChange={e=>setA(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm"/></div><div><label className="aaa-meta">ADMIN TEAM B</label><input value={b} list={NICKNAMES_DATALIST_ID} onChange={e=>setB(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm"/></div><button onClick={()=>onSave(a,b)} className="col-span-2 rounded-xl border border-amber-300/20 bg-amber-300/5 py-2.5 text-[9px] font-black uppercase tracking-[.18em] text-amber-200">Uložit sestavy</button></div>}
