...
export const createSupportCheckout = createServerFn({ method: "POST" })
  .inputValidator((data: {
    ...
    if (!Number.isInteger(data.amountInCents) || data.amountInCents < 5000) {
      throw new Error("Minimální příspěvek je 50 Kč");
    }
    if (data.amountInCents > 5000000) {
      throw new Error("Maximální příspěvek je 50 000 Kč");
    }
    return data;
  })
  .handler(async ({ data }): Promise<CheckoutSessionResult> => {
    try {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        payment_intent_data: { description: "Podpora Chmeloví Sportovci" },
        ...(data.customerEmail && { customer_email: data.customerEmail }),
        ...(nickname && { metadata: { supporterNickname: nickname } }),
      });
